...
const AffiliateRedirect = lazy(() => import('./components/AffiliateRedirect'));
...
const App: React.FC = () => {
  ...
  if (path === '/claim-opportunity') {
    ...
    const token = params.get('token');   // JWT signed (new)
    const id = params.get('id');         // Legacy (retro-compat 30d)
    return <ClaimOpportunity opportunityId={id} claimToken={token} />;
  }

  // PR6: Handle /r/:code affiliate links
  ...
  if (path === '/commercial-onboarding' || path.startsWith('/commercial-onboarding')) {
    return <CommercialOnboarding />;
  }
