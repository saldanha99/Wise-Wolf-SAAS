...
const TrialFeedbackForm: React.FC<TrialFeedbackFormProps> = ({
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            const { data: rpcResult, error: rpcError } = await supabase.rpc('record_trial_feedback', {
                p_opportunity_id: opportunityId,
                p_teacher_id: teacherId,
                p_recommended_level: level,
                p_recommended_plan: plan,
                p_interest_score: interest,
            ...
            if (rpcResult && !rpcResult.success) {
                throw new Error(rpcResult.error || 'Erro desconhecido ao salvar feedback.');
            }
