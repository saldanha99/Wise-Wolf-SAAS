                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 max-w-md w-full shadow-2xl relative">
                        ...
                        <form onSubmit={handleAddModule} className="space-y-4">
                            ...
                            {newModule.resource_type === 'video' ? (
                                ...
                                <div>
                                    <label className="text-xs font-bold uppercase text-slate-500">Upload do PDF</label>
                                    <div className="mt-2 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl p-6 text-center hover:border-purple-400 transition-colors cursor-pointer relative">
                                        <input
                                            type="file"
                                            accept=".pdf"
                                            onChange={e => {
                                        ...
                                        {pdfFile ? (
                                            ...
                                            <p className="text-xs text-slate-400">Clique ou arraste o PDF aqui</p>
                                        )}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex gap-2 mt-6">
                                <button type="button" onClick={() => { setIsAdding(false); setPdfFile(null); }} className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-500 font-bold rounded-xl hover:bg-slate-200 transition-colors">Cancelar</button>
            ...
            {assignModalModule && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 max-w-md w-full shadow-2xl">
                        ...
                        <p className="text-sm text-slate-500 mb-6">Selecione os professores que devem completar este treinamento. Ao concluir, serão remunerados pelo valor da aula.</p>
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                            {teachers.map(t => (
                                <div
                                    ...
                                    className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${selectedTeachers.includes(t.id) ? 'bg-purple-50 dark:bg-purple-900/20 border-2 border-purple-500' : 'bg-slate-50 dark:bg-slate-800 border-2 border-transparent hover:border-slate-200'}`}
                                >
                                    <div className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-colors ${selectedTeachers.includes(t.id) ? 'bg-purple-600 border-purple-600' : 'border-slate-300'}`}>
                                        {selectedTeachers.includes(t.id) && <CheckCircle size={14} className="text-white" />}
                                    </div>
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{t.name || t.full_name}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex gap-2 mt-6">
                            <button onClick={() => { setAssignModalModule(null); setSelectedTeachers([]); }} className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-500 font-bold rounded-xl hover:bg-slate-200 transition-colors">Cancelar</button>
                            <button
