        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-brand-muted text-xs mt-1">Defina os tiers, limites e preços da plataforma.</p>
                </div>
                <button
                ...
                {loading ? (
                    ...
                    <div key={plan.id} className="group bg-brand-surface border border-brand-border rounded-[2.5rem] p-6 relative overflow-hidden transition-all hover:shadow-xl hover:border-blue-200">
                        ...
                        <div className="flex justify-between items-start mb-4 relative z-10">
                            <div>
                                <h4 className="font-black text-brand-text text-lg">{plan.name}</h4>
                                <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider mt-1 ${plan.active ? 'bg-emerald-100 text-emerald-600' : 'bg-brand-surface-2 text-brand-muted'}`}>
                        ...
                        <div className="pt-4 border-t border-slate-50 dark:border-brand-border relative z-10">
                            <div className="flex items-baseline gap-1">
                                <span className="text-2xl font-black text-brand-text">R$ {plan.price}</span>
                                <span className="text-xs font-bold text-brand-muted">/mês</span>
                            </div>
                            {plan.price_yearly && <p className="text-[10px] text-brand-muted mt-1">Ou R$ {plan.price_yearly} /ano</p>}
                        </div>
                    </div>
                ))}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-surface/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-brand-surface w-full max-w-2xl rounded-[2.5rem] p-8 border border-brand-border dark:border-brand-border shadow-2xl overflow-y-auto max-h-[90vh]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-brand-muted uppercase tracking-widest">Armazenamento (GB)</label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-brand-muted uppercase tracking-widest">Max Alunos</label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-brand-muted uppercase tracking-widest">Max Admin Users</label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-brand-muted uppercase tracking-widest">Preço Anual (R$)</label>
                                <input
                                    type="number"
                            ...
                            <div className="col-span-1 md:col-span-2 flex items-center gap-3">
                                ...
                                >
                                    <div className={`absolute top-1 w-4 h-4 bg-brand-surface rounded-full transition-transform ${formData.active ? 'left-7' : 'left-1'}`} />
                                </button>
                                <span className="text-sm font-bold text-brand-muted dark:text-brand-muted">Plano Ativo para Venda?</span>
                            </div>
                        </div>
