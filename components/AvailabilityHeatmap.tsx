...
const AvailabilityHeatmap: React.FC = () => {
  ...
  const getColorClass = (value: number) => {
    ...
    if (value === 0) return 'bg-gray-100 dark:bg-brand-surface-2';
    ...
    <div className="bg-brand-surface p-4 md:p-8 rounded-[2rem] md:rounded-3xl border border-gray-100 dark:border-brand-border shadow-sm overflow-hidden">
      <div className="mb-8 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-brand-muted mt-1">Janelas de oportunidade para novos alunos.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3 bg-gray-50 dark:bg-brand-surface-2/50 p-3 rounded-2xl border dark:border-brand-border">
            <span className="text-[10px] text-gray-400 dark:text-brand-muted uppercase font-black">Legenda:</span>
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-1.5">
                 <div className="w-3 h-3 rounded-sm bg-gray-800 dark:bg-slate-600" />
                 <span className="text-[9px] text-gray-500 dark:text-brand-muted font-bold">PICO</span>
              </div>
              <div className="flex items-center gap-1.5">
                 <div className="w-3 h-3 rounded-sm bg-gray-100 dark:bg-brand-surface-2" />
                 <span className="text-[9px] text-gray-500 dark:text-brand-muted font-bold">SEM VAGA</span>
              </div>
              <div className="flex items-center gap-1.5">
                 <div className="w-3 h-3 rounded-sm bg-emerald-500" />
                 <span className="text-[9px] text-gray-500 dark:text-brand-muted font-bold">LIVRE</span>
              </div>
            </div>
        </div>
      </div>
      ...
      <div className="overflow-x-auto -mx-4 px-4 scrollbar-hide">
        <table className="w-full border-separate border-spacing-1 min-w-[600px]">
          <thead>
            <tr>
              <th className="p-2 text-[10px] text-gray-400 dark:text-brand-muted font-black uppercase w-16">Hora</th>
              {DAYS.map(day => (
                <th key={day} className="p-2 text-[10px] text-gray-500 dark:text-brand-muted font-black uppercase tracking-widest">{day}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {TIMES.map((time, rowIdx) => (
              <tr key={time}>
                <td className="p-2 text-[10px] text-gray-400 dark:text-brand-muted text-center font-bold">{time}</td>
