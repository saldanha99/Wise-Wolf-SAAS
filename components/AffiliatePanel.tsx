...
const AffiliatePanel: React.FC<AffiliatePanelProps> = ({ user }) => {
    ...
    const isTeacher = user.role === UserRole.TEACHER || user.role === UserRole.SCHOOL_ADMIN || user.role === UserRole.COMMERCIAL;
    const isStudent = user.role === UserRole.STUDENT;

    // Novo link usando código
    const affiliateLink = affiliateCode 
        ? `${APP_BASE_URL}/r/${affiliateCode}`
        : `${APP_BASE_URL}/matricula?ref=${user.id}`; // fallback legacy
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            // 2. Obter ou gerar código via RPC
            const roleForRpc = isTeacher ? 'TEACHER' : 'STUDENT';
            const { data: codeResult } = await supabase.rpc('generate_affiliate_code', {
                p_owner_id: user.id,
                p_owner_type: roleForRpc,
                p_tenant_id: tenantId
            });

            if (codeResult?.success && codeResult?.code) {
                setAffiliateCode(codeResult.code);
            }

            // 3. Obter configurações de recompensa
            const { data: settings } = await supabase
                .from('tenant_referral_settings')
                .select('teacher_reward_brl, student_reward_brl')
                .eq('tenant_id', tenantId)
                .single();
            
            const bonus = isTeacher 
                ? (settings?.teacher_reward_brl || 50) 
                : (settings?.student_reward_brl || 30);
            
            setBonusAmount(bonus);

            // 4. Obter indicados (fallback temporário lendo de profiles legados para manter UI enquanto converte tudo)
            const referrerColumn = isTeacher ? 'referrer_teacher_id' : 'referrer_student_id';
                ...
                <div className="relative z-10 p-8 md:p-10">
                    <div className="flex items-start justify-between mb-6">
                        <div>
                            ...
                            <p className="text-emerald-100/90 font-medium text-base max-w-sm">
                                ...
                                <span className="font-black text-white">R$ {bonusAmount.toFixed(2)}</span> pra você!
                            </p>
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-6">
                        {[
                            ...
                            { step: '3', label: `Ganhe R$${bonusAmount} por matrícula` },
                        ].map(({ step, label }) => (
