                      ...
                      >
                        {booking ? (
                          <div
                            ...
                            className={`w-full h-full rounded-xl p-2.5 cursor-pointer hover:scale-[1.02] transition-transform shadow-md shimmer relative overflow-hidden group/card ${booking.isExperimental
                              ? 'bg-indigo-600 shadow-indigo-200 ring-2 ring-indigo-100'
                              : 'bg-slate-900 dark:bg-slate-800'
                              }`}
                          >
                            {booking.isExperimental && (
                              <div className="absolute top-0 right-0 px-1.5 py-0.5 bg-white/20 backdrop-blur-md rounded-bl-lg">
                                <span className="text-[8px] font-black text-white">{booking.dateLabel}</span>
                              </div>
                            )}
                            <div className="relative z-10 flex flex-col justify-between h-full">
                              <div className="flex items-center gap-2">
                                <div className={`w-5 h-5 rounded-md flex items-center justify-center text-[9px] font-bold ${booking.isExperimental ? 'bg-white text-indigo-600' : 'bg-white/10 text-white'
                                  }`}>
                                  {booking.student[0]}
                                </div>
                              ...
                              <span className={`text-[9px] font-medium uppercase tracking-wider truncate ${booking.isExperimental ? 'text-indigo-200' : 'text-slate-400'
                                }`}>
                                {booking.isExperimental ? 'AULA EXPERIMENTAL' : `${booking.module} • ${booking.type === 'REPOSIÇÃO' ? 'REPO' : 'FIXO'}`}
                              </span>
                            </div>
                          </div>
                        ) : (
