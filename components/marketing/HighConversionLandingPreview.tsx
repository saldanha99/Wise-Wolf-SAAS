...
const HighConversionLandingPreview: React.FC<HighConversionLandingPreviewProps> = ({
    ...
    return (
        <div className="w-full h-full bg-brand-surface overflow-y-auto font-sans text-brand-text scrollbar-hide">
                ...
                <div className={`max-w-6xl mx-auto relative z-10 grid gap-12 items-center ${isMobile ? 'grid-cols-1' : 'lg:grid-cols-2'}`}>
                    ...
                    <div className="bg-brand-surface text-brand-text p-6 md:p-8 rounded-[2rem] shadow-2xl relative">
                        ...
                        <div className="text-center mb-6">
                            <h3 className="text-2xl font-black text-brand-text">Garanta sua Vaga</h3>
                            <p className="text-sm text-brand-muted font-medium mt-1">Preencha para agendar sua experimental</p>
                        </div>
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            ...
                            <div className="flex items-center justify-center gap-2 text-[10px] text-brand-muted font-bold uppercase tracking-wider mt-4">
            ...
            {company_logos && company_logos.length > 0 && (
                <div className="bg-slate-950 py-10 border-b border-slate-900 overflow-hidden">
                    <p className="text-center text-brand-muted text-xs font-bold uppercase tracking-widest mb-6">Nossos alunos trabalham em</p>
            ...
            <section className="bg-brand-surface-2 border-b border-brand-border py-12">
                <div className="max-w-6xl mx-auto px-6 text-center">
                    {stats && stats.length > 0 ? (
                        ...
                        <>
                            <p className="text-xs font-black text-brand-muted uppercase tracking-[0.2em] mb-6">Método validado por grandes empresas</p>
            ...
            <section className="py-20 bg-brand-surface relative overflow-hidden">
                <div className="max-w-6xl mx-auto px-6">
                    <div className="text-center mb-16">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-brand-text">Por que somos diferentes?</h2>
                    </div>
                        ...
                        <div className="grid md:grid-cols-3 gap-8">
                            {benefits.map((item, i) => (
                                <div key={i} className="bg-brand-surface-2 p-8 rounded-[2rem] border border-brand-border hover:border-blue-200 transition-colors group">
                                    ...
                                    <p className="text-brand-muted font-medium leading-relaxed">{item.description}</p>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <div className="grid md:grid-cols-2 gap-8 relative z-10">
                            ...
                            <div className="bg-brand-surface border-2 border-brand-border rounded-[2rem] p-8 opacity-80 hover:opacity-100 transition-opacity">
                                ...
                                <ul className="space-y-6">
                                    <li className="flex items-start gap-4 text-brand-muted font-medium">
            ...
            {teachers && teachers.length > 0 && (
                <section className="bg-brand-surface-2 py-24 px-6">
                    <div className="max-w-6xl mx-auto">
                        <div className="text-center mb-16">
                            <span className="text-purple-600 font-black uppercase tracking-widest text-xs mb-2 block">Quem vai te ensinar</span>
                            <h2 className="text-3xl md:text-5xl font-black text-brand-text">Conheça nossos especialistas</h2>
                        </div>

                        <div className="grid md:grid-cols-3 gap-8">
                            {teachers.map((teacher, i) => (
                                <div key={i} className="bg-brand-surface p-6 rounded-3xl shadow-xl shadow-slate-200 border border-brand-border hover:-translate-y-2 transition-transform duration-300">
                                    <div className="flex items-center gap-4 mb-6">
                                        <div className="w-16 h-16 rounded-full bg-slate-200 overflow-hidden">
                                            {teacher.photo ? <img src={teacher.photo} className="w-full h-full object-cover" /> : <User size={32} className="m-auto mt-4 text-brand-muted" />}
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-brand-text text-lg">{teacher.name}</h4>
                                            <p className="text-purple-600 text-xs font-bold uppercase">{teacher.bio}</p>
                                        </div>
                                    </div>
                                    {teacher.media_url && (
                                        <div className="bg-brand-surface-2 rounded-xl p-3 flex items-center gap-3 cursor-pointer hover:bg-slate-200 transition-colors">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-[10px] text-brand-muted font-bold uppercase">Ouvir apresentação</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
            ...
            <section className="bg-brand-surface py-20 px-6 text-white relative overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {(targetAudience && targetAudience.length > 0 ? targetAudience : [
                            ...
                            <div key={i} className="bg-brand-surface-2/50 p-8 rounded-3xl border border-brand-border hover:bg-brand-surface-2 transition-colors">
            ...
            <section className="bg-brand-surface-2 py-20 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="text-center mt-12">
                        <button
                            ...
                            className="bg-brand-surface hover:bg-brand-surface-2 text-white px-8 py-4 rounded-xl font-black text-sm uppercase tracking-widest shadow-lg shadow-slate-900/30 transform hover:scale-105 transition-all">
            ...
            <section className="bg-brand-surface text-white py-24 px-6 relative overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    <div className="text-center mb-16">
                        ...
                        <p className="text-brand-muted font-medium max-w-2xl mx-auto">Veja o que nossos alunos estão dizendo sobre a metodologia.</p>
                    </div>

                    <div className="grid md:grid-cols-3 gap-6">
                        {(testimonials && testimonials.length > 0 ? testimonials : [1, 2, 3]).map((t: any, i) => (
                            testimonials && testimonials.length > 0 ? (
                                <div key={i} className="bg-brand-surface-2 p-8 rounded-3xl border border-brand-border relative hover:-translate-y-1 transition-transform">
                                    <div className="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-brand-muted uppercase tracking-wider">{t.role}</p>
                                        </div>
                                    </div>
            ...
            <section className="bg-red-600 py-24 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto px-6 text-center text-white relative z-10">
                    ...
                    <button className="bg-brand-surface text-red-600 px-12 py-5 rounded-full font-black text-lg uppercase tracking-widest shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:scale-105 hover:shadow-[0_20px_60px_rgba(0,0,0,0.4)] transition-all">
            ...
            {/* Footer */}
            <footer className="bg-slate-950 text-brand-muted py-12 text-center text-xs font-bold uppercase tracking-widest border-t border-slate-900">
