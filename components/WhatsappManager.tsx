...
const WhatsappManager: React.FC<WhatsappManagerProps> = ({ role, userId }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-lg border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center max-w-sm mx-auto animate-in fade-in zoom-in duration-500">
            ...
            <div className="flex items-center gap-2 mb-8 bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-800">
                <div className={`w-2 h-2 rounded-full ${statusText === 'Conectado' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                <span className="font-mono text-[10px] uppercase tracking-widest text-slate-500">{instanceName}</span>
            </div>

            {/* STATUS TEXT DISPLAY */}
            <div className="mb-8 w-full">
                <div className="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 px-1">
                    <span>Status</span>
            ...
            <div className="w-full space-y-4">
                ...
                {statusText === 'Aguardando QR Code' && qrCode && (
                    <div className="p-4 bg-white rounded-[2rem] border-2 border-blue-100 shadow-inner mb-4 animate-in zoom-in">
                        <img src={qrCode} alt="QR Code WhatsApp" className="w-full h-auto rounded-xl mix-blend-multiply" />
                        <p className="text-[10px] text-slate-400 mt-3 font-bold uppercase tracking-widest animate-pulse">Aponte a câmera do seu celular</p>
                    </div>
                )}
                ...
                {statusText === 'Desconectado' && (
                    <button
                        ...
                        disabled={loading}
                        className="w-full py-4 bg-slate-900 dark:bg-white hover:opacity-90 text-white dark:text-slate-900 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-xl shadow-slate-900/20 active:scale-95 flex items-center justify-center gap-2"
                    >
                ...
                {statusText === 'Conectado' && (
                    <button
                        onClick={logout}
                        disabled={loading}
                        className="w-full py-4 bg-white border-2 border-rose-100 hover:bg-rose-50 text-rose-500 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-lg shadow-rose-500/5 active:scale-95 flex items-center justify-center gap-2"
                    >
