...
const StudentBilling: React.FC<StudentBillingProps> = ({ user }) => {
  ...
  const fetchBillingData = async () => {
    setLoading(true);
    try {
      // 1. Fetch Profile (Contract Info)
      const { data: profile, error: profileError } = await supabase
        .from('profiles')
        .select('id, full_name, module, monthly_fee, status, due_day, status_financial')
        .eq('id', user.id)
        .single();
      ...
      // 2. Fetch Payments History (Real-time from student_payments)
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 relative font-sans">
      ...
      {alertStatus === 'NORMAL' && payments.some(p => p.status === 'PENDING') && (
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/30 rounded-2xl p-4 flex items-center gap-4">
          <div className="p-2 bg-amber-100 text-amber-600 rounded-lg">
            <Info size={20} />
          </div>
          <div>
            <p className="text-amber-800 dark:text-amber-200 text-xs font-bold uppercase tracking-wide">Fatura Disponível</p>
            <p className="text-amber-700 dark:text-amber-400 text-xs">Sua mensalidade deste mês já está disponível para pagamento.</p>
          </div>
        </div>
      )}
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <span className={`flex items-center gap-2 font-black text-sm uppercase tracking-wide ${billingInfo?.status_financial === 'ACTIVE' ? 'text-emerald-500' : 'text-amber-500'}`}>
                  <CheckCircle size={14} /> {billingInfo?.status_financial === 'ACTIVE' ? 'Ativo & Regular' : billingInfo?.status_financial || 'Sem Plano'}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xl font-black text-slate-700 dark:text-slate-200 tracking-tight">Dia {billingInfo?.due_day || '??'}</span>
                </div>
              </div>
            </div>
