    ...
    const handleTestMessage = async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                const errorStr = JSON.stringify(res.data) + (res.error || '');
                if (errorStr.includes('Connection Closed')) {
                    alert('Seu WhatsApp foi desconectado (provavelmente pelo seu celular). O sistema atualizará o status. Por favor, remova a conexão e gere um novo QR Code.');
                    checkStatus();
                } else {
                ...
                <div className="max-w-md mx-auto text-center py-8">
                    ...
                    <div className="flex gap-2 mb-4">
                        ...
                        <button
                            ...
                            className={`px-6 py-3 rounded-xl font-bold text-sm text-white transition-all flex items-center gap-2 ${isRecoveryMode
                                ? 'bg-blue-600 hover:bg-blue-700'
                                : 'bg-emerald-500 hover:bg-emerald-600'
                                } disabled:opacity-50`}
                        >
                            <RefreshCw size={16} className={loading ? 'animate-spin' : ''} />
                            {isRecoveryMode ? 'Importar' : 'Criar'}
                        </button>
                    </div>
                        ...
                        {connectionStatus === 'connected' ? (
                                ...
                                {['DIRECTOR', 'ADMIN', 'SUPER_ADMIN', 'SCHOOL_ADMIN', 'FRANCHISEE'].includes(user.role) && (
                                    <div className="mt-8 w-full border-t border-slate-100 dark:border-slate-800 pt-8 text-left grid grid-cols-1 gap-6">
                                        ...
                                        <GroupSelector
                                            user={user}
                                            instanceName={instance.instance_name}
                                            label="Grupo de Avisos (Leads / Aceites)"
                            ...
                            <div className="text-center">
                                ...
                                <button
                                    ...
                                    className="bg-tenant-primary text-white px-8 py-3 rounded-xl font-black text-sm uppercase tracking-widest hover:scale-105 transition-all shadow-lg"
                                >
                                    Gerar QR Code
                                </button>
                            </div>
