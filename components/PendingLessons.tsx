  ...
  const handleSave = async (formData: any) => {
    ...
    try {
      const entries = Object.keys(formData).map(lessonId => {
        ...
        // EXPIRED Logic (Hard Limit)
        // If lesson is from previous month (or older) AND we are past the 5th (default cutoff), mark as EXPIRED
        const lessonDate = new Date(lesson.rawDate);
        const now = new Date();
        // Check if lesson is strictly from a previous month (not just 30 days ago, but actual calendar month)
        const isPreviousMonth = lessonDate.getMonth() < now.getMonth() || lessonDate.getFullYear() < now.getFullYear();
        // TODO: Fetch real cutoff from tenant settings. Defaulting to 5.
        const CUTOFF_DAY = 5;
        const isPastCutoff = now.getDate() > CUTOFF_DAY;

        let finalPresence = data.type || 'COMPLETED';
        if (isPreviousMonth && isPastCutoff) {
          finalPresence = 'EXPIRED';
        }
