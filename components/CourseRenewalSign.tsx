...
export default function CourseRenewalSign() {
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8"><div className="mx-auto max-w-xl space-y-4">
    ...
    <section className="rounded-3xl bg-white p-6 shadow-sm"><p className="text-sm text-slate-600">Olá, <b>{data.student_name}</b>. Confira as condições aprovadas:</p>
      <dl className="mt-5 grid grid-cols-2 gap-3 text-sm"><div className="rounded-xl bg-slate-50 p-3"><dt className="text-slate-500">Período</dt><dd className="font-black">6 meses</dd></div><div className="rounded-xl bg-slate-50 p-3"><dt className="text-slate-500">Mensalidade</dt><dd className="font-black">{money(data.monthly_fee_cents)}</dd></div><div className="rounded-xl bg-slate-50 p-3"><dt className="text-slate-500">Frequência</dt><dd className="font-black">{data.classes_per_week}x por semana</dd></div><div className="rounded-xl bg-slate-50 p-3"><dt className="text-slate-500">Vigência</dt><dd className="font-black">{date(data.contract_start)} a {date(data.service_end_date)}</dd></div></dl>
      ...
      <p className="mt-3 text-xs text-slate-500">Esta renovação prorroga o contrato anterior. As demais cláusulas continuam válidas; somente vigência, parcelas, valor e frequência ficam confirmados conforme o resumo acima.</p>
