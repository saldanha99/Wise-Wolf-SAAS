...
const TeacherFinancials: React.FC<TeacherFinancialsProps> = ({ user, tenantId }) => {
    ...
    const fetchFinancialData = async () => {
        ...
        `)
            ...
            // Filter for display (we show all, but we will highlight paid vs unpaid in UI)
            setLessons(logs || []);
    ...
    // Calculation Helper - Truth Table
    // Paid: COMPLETED, STUDENT_ABSENCE
    // Unpaid: TEACHER_ABSENCE, EXPIRED, REPOSICAO
    const isLessonPaid = (log: any) => {
        const isTeacherFault = log.presence === 'TEACHER_ABSENCE' || log.presence === 'Falta do Professor';
        const isExpired = log.presence === 'EXPIRED';
        const isMakeup = log.subtype === 'REPOSIÇÃO';

        return !isTeacherFault && !isExpired && !isMakeup;
    };

    const handleConfirm = async () => {
        try {
            const totalAmount = lessons.reduce((acc, log) => {
                if (!isLessonPaid(log)) return acc;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="mt-4 flex items-center gap-2 text-[10px] font-bold text-emerald-600/60 bg-emerald-50 dark:bg-emerald-900/20 px-3 py-1 rounded-full w-fit">
                        <TrendingUp size={12} /> {lessons.filter(l => isLessonPaid(l)).length} Aulas remuneradas
                    </div>
                </div>
                                ...
                                <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <td className="px-8 py-6">
                                        <span className={`text-sm font-black ${!isLessonPaid(log)
                                            ...
                                            R$ {!isLessonPaid(log) ? '0,00' : (user.hourlyRate || 7.50).toFixed(2)}
                                        </span>
                                    </td>
                                </tr>
                            ))}
