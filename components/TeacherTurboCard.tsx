...
// Turbo é uma ofensiva contínua de 30 dias e exige a carteira mínima do tenant
// (7 na Wise Wolf desde 09/09/2026; 10 nas demais escolas — quem decide é o banco).
...
const TeacherTurboCard: React.FC<Props> = ({ teacherId }) => {
  ...
  // Regra 09/09/2026: na Wise Wolf o turbo destrava a partir de 7 alunos ativos
  ...
  const bloqueio: Record<string, string> = {
    carteira: 'O Turbo fica disponível a partir de 7 alunos ativos na carteira.',
          ...
          <div className="space-y-2">
            {studentsMissing > 0 && (
              <div className="text-xs font-bold rounded-xl px-3 py-2 bg-brand-surface-2 border border-brand-border text-brand-text">
                🎯 Faltam <b>{studentsMissing} aluno{studentsMissing === 1 ? '' : 's'}</b> para você poder ativar o turbo: o benefício destrava a partir de <b>7 alunos na carteira</b> (hoje você tem {studentsActive}). Quanto mais assiduidade e qualidade, mais alunos a escola te envia.
              </div>
            )}
            {motivo && (
