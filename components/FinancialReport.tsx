...
const FinancialReport: React.FC<FinancialReportProps> = ({ role, tenantId }) => {
  ...
  const fetchFinancialData = async () => {
    ...
    try {
      ...
      // 5. Fetch Real Cash Flow (Official Financial Transactions) for Totals & List
      // This is more reliable as it's the "School Cash" ledger
      const { data: transactions } = await supabase
        .from('financial_transactions')
        .select('id, amount, type, created_at, reference_id, description, category')
        .eq('tenant_id', tenantId)
        .eq('category', 'MENSALIDADE')
        .gte('created_at', startOfMonth)
        .lt('created_at', endOfMonth);

      // 5.1 Fetch Profiles for Transactions (Manual Join to be safe)
      // 5.1 Fetch Profiles for Transactions (Manual Join to be safe)
      const referenceIds = (transactions || [])
        .map(t => t.reference_id)
        .filter(id => id && typeof id === 'string'); // Ensure only valid IDs
      ...
      if (referenceIds.length > 0) {
        ...
        if (transProfiles) {
          profileMap = new Map(transProfiles.map(p => [p.id, p.full_name]));
        }
      }

      // 6. Fetch PENDING Student Payments for the List (Table)
      // Only get what is NOT paid yet, because paid ones come from transactions
      ...
      // Calculations
      // Revenue based on OFFICIAL CAIXA (financial_transactions)
      ...
      // Map real payments to UI (Transactions)
      const mappedTransactions = (transactions || []).map((t: any) => ({
        id: t.id,
        name: profileMap.get(t.reference_id) || 'Aluno Desconhecido',
        amount: t.amount || 0,
        status: 'RECEIVED',
      ...
      const mappedPending = (pendingPayments || []).map((p: any) => ({
        id: p.id,
        name: p.profiles?.full_name || 'Aluno Desconhecido',
        amount: p.value || 0,
      ...
      // Merge and Sort
      const mixedReceipts = [...mappedTransactions, ...mappedPending].sort((a, b) => {
        // Sort by date (string dd/mm/yyyy needs conversion for sort, strictly speaking)
        // Actually, let's just reverse because we want newest on top?
        // Or parsing.
      ...
      setStudentReceipts(mixedReceipts);
