...
const TrialsToContracts: React.FC<TrialsToContractsProps> = ({ tenantId, user }) => {
    ...
    // Enrollment Fee & Start Date (Matching official generator)
    ...
    const fetchData = async () => {
        ...
        if (oppIds.length > 0) {
            ...
            // 3. Enrollment links for those opportunities (from enrollment_intents)
    ...
    const openWizard = (opp: Opportunity) => {
        ...
        // Reset enrollment fee & start date
        setChargeEnrollmentFee(true);
        setEnrollmentFee(49);
    ...
    const handleGenerateLink = async () => {
        ...
        try {
            ...
            // Create signed offer via Edge Function (falls back to legacy)
            ...
            // Save to enrollment_intents (replaces enrollment_links for new flow)
            const { error: intentErr } = await supabase
                ...
                .insert({
                    ...
                    professor_id2: selectedProfessor2 || null,
                    start_date: startDate,
                    enrollment_fee: chargeEnrollmentFee ? enrollmentFee : 0,
            ...
            if (intentErr) {
                console.warn('enrollment_intents insert failed, trying legacy enrollment_links:', intentErr);
                // Fallback: save to legacy enrollment_links table
                await supabase
                    .from('enrollment_links')
                    .insert({
                        tenant_id: tenantId,
                        opportunity_id: wizardOpp.id,
                        link_token: `trial_${wizardOpp.id}_${Date.now()}`,
                        link_url: url,
                        student_name: wizardOpp.student_name,
                        student_phone: wizardOpp.student_phone,
                        professor_id: selectedProfessor || null,
    ...
    const handleMarkLost = async () => {
        if (!lostOpp || !user) return;
        setSavingLost(true);
        try {
            // Use atomic RPC instead of direct update
            const { data: result, error: rpcError } = await supabase.rpc('mark_opportunity_lost', {
                p_opportunity_id: lostOpp.id,
                p_actor_id: user.id,
                p_reason: lostReason || 'Não especificado',
            });

            if (rpcError) throw rpcError;

            if (!result?.success) {
                throw new Error(result?.message || 'Não foi possível marcar como perdido.');
            }
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                {/* Enrollment Fee & Start Date Controls */}
                                <div className="mt-4 pt-4 border-t border-slate-100 space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="text-[10px] font-bold uppercase text-slate-400 mb-1 block italic">Início das Aulas</label>
                                            <div className="relative">
                                                <input
                                                    type="date"
                                                    value={startDate}
                                                    onChange={(e) => setStartDate(e.target.value)}
                                                    className="w-full px-3 py-2.5 bg-slate-50 border-none rounded-xl font-bold text-xs text-slate-700 appearance-none outline-none focus:ring-2 focus:ring-blue-500"
                                                />
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-1">
                                                <label className="flex items-center gap-2 text-[10px] font-bold uppercase text-slate-400 cursor-pointer hover:text-blue-500 transition-colors italic">
                                                    <input
                                                        type="checkbox"
                                            ...
                                            {chargeEnrollmentFee && (
                                                <div className="relative animate-in fade-in slide-in-from-top-1">
                                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-black text-[10px]">R$</span>
                                                    <input
                                                        type="number"
                                                        value={enrollmentFee}
                                                        onChange={(e) => setEnrollmentFee(Number(e.target.value))}
                                                        className="w-full pl-8 pr-4 py-2.5 bg-blue-50 border border-blue-100 rounded-xl font-black text-xs text-blue-700 outline-none focus:ring-2 focus:ring-blue-500"
                                                    />
                                                </div>
                                            )}
                                        </div>
                            ...
                            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="relative z-50">
                                        <label className="text-[10px] font-bold uppercase text-slate-400 mb-1 block italic">Professor 1</label>
                                    ...
                                    <div className="relative z-50">
                                        <label className="text-[10px] font-bold uppercase text-slate-400 mb-1 block italic">Professor 2 (Opcional)</label>
                                        <div className="relative group">
                                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                            <input
                                                type="text"
                                                placeholder="Buscar Professor 2..."
                                                value={selectedProfessor2 ? (teachers.find(p => p.id === selectedProfessor2)?.full_name || '') : professorSearch2}
                                                onChange={(e) => {
                                                    setProfessorSearch2(e.target.value);
                                                    setSelectedProfessor2('');
                                                    setShowProfessorList2(true);
                                                }}
                                                onFocus={() => { setProfessorSearch2(''); setShowProfessorList2(true); }}
                                                className="w-full pl-10 pr-4 py-3 bg-white border border-dashed border-slate-300 rounded-xl text-sm font-medium text-slate-700 outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 transition-all shadow-sm cursor-pointer"
                                            />
                                            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={16} />

                                            {showProfessorList2 && (
                                                <>
                                                    <div className="fixed inset-0 z-40" onClick={() => setShowProfessorList2(false)} />
                                                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 overflow-y-auto z-50">
                                                        {teachers.filter(p => !professorSearch2 || p.full_name.toLowerCase().includes(professorSearch2.toLowerCase())).length > 0 ? (
                                                            teachers
                                                                .filter(p => !professorSearch2 || p.full_name.toLowerCase().includes(professorSearch2.toLowerCase()))
                                                                .map(p => (
                                                                    <button
                                                                        key={`p2-${p.id}`}
                                                                        onClick={() => {
