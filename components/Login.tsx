...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent, data: { email: string, password: string }) => {
    ...
    try {
      ...
      // 2.5 Ensure Profile was found or created
      if (!profile) {
        // If we reach here, the auth.users exists but public.profiles doesn't
        // and it's not a mock account. Orphaned auth user.
        await supabase.auth.signOut();
        throw new Error('Conta desativada ou perfil não encontrado. Contate o suporte.');
      }

      // 2.6 Fix False Positives: Check if Student actually PAID but status is not ACTIVE
