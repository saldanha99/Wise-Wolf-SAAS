...
export function SignInCard2({ onLogin, isLoading, error }: SignInCard2Props) {
  ...
  const handlePasswordRecovery = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) {
      setRecoveryStatus("Informe um e-mail válido para recuperar a senha.");
      return;
    }
    ...
    setRecoveryStatus(
      recoveryError
        ...
        : "Se a conta existir, enviaremos um link seguro para esse e-mail.",
    );
  };
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <button
                    ...
                    className="text-blue-400 hover:text-blue-300 font-medium transition-colors disabled:cursor-wait disabled:opacity-60"
                  >
                    {recoveryLoading ? "Solicitando…" : "Recuperar senha?"}
                  </button>
                </div>
                ...
                {recoveryStatus && (
                  <p
                    role="status"
                    aria-live="polite"
                    className="rounded-xl border border-blue-400/20 bg-blue-400/10 px-4 py-3 text-xs text-blue-100"
                  >
                    {recoveryStatus}
                  </p>
                )}
...
273:                                             { label: 'Professor', email: 'professor.demo@wisewolf.com', pass: '123456' },
274:                                             { label: 'Aluno', email: 'aluno@wisewolf.com', pass: '123456' },
275:                                             { label: 'Super Admin', email: 'admin@educore.io', pass: '123456' }
276:                                         ].map((demo, i) => (
277:                                             <button
278:                                                 key={i}
279:                                                 type="button"
280:                                                 onClick={() => {
281:                                                     setEmail(demo.email);
