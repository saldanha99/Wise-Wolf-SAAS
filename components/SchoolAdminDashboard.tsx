...
const SchoolAdminDashboard: React.FC<SchoolAdminDashboardProps> = ({ teachers, tenantId, onViewTeacherSchedule, userRole }) => {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'training' | 'registration' | 'payments' | 'contracts' | 'recruiting' | 'commercial'>('analytics');
  const [showTrialScheduler, setShowTrialScheduler] = useState(false);
  const [recruitType, setRecruitType] = useState<'teacher' | 'commercial'>('teacher');
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 font-sans">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'commercial' ? 'bg-white dark:bg-slate-900 text-slate-800 dark:text-white shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
            >
              Equipe Vendas
            </button>
            <button
      ...
      {activeTab === 'recruiting' && (
        <div className="max-w-md mx-auto space-y-6 animate-in zoom-in-95 duration-300">
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit mx-auto">
             <button 
              onClick={() => setRecruitType('teacher')}
              className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${recruitType === 'teacher' ? 'bg-white dark:bg-slate-900 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
               Professor
             </button>
             <button 
              onClick={() => setRecruitType('commercial')}
              className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${recruitType === 'commercial' ? 'bg-white dark:bg-slate-900 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
               Vendedor
             </button>
          </div>
      ...
      {activeTab === 'commercial' && <CommercialManagement tenantId={tenantId} />}
