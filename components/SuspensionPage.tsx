...
const SuspensionPage: React.FC<SuspensionPageProps> = ({ user, onLogout }) => {
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-4 font-sans">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-md w-full p-8 text-center border border-slate-200 dark:border-slate-700">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-700 mb-8 text-left">
                    <div className="flex items-center gap-3 mb-2">
                        <AlertCircle size={18} className="text-amber-500" />
                        <span className="font-bold text-slate-700 dark:text-slate-300 text-sm">Status Atual:</span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-500 pl-8">
                ...
                <div className="space-y-3">
                    ...
                    <button
                        onClick={onLogout}
                        className="w-full py-3 bg-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 font-bold text-sm transition-colors"
                    >
                        Sair da Conta
                    </button>
                </div>
