...
-- SCHEMA STANDARDIZATION FOR APPOINTMENTS AND CLASS LOGS (V4)

-- 1. Appointments Table: Add missing columns and migrate data
DO $$ 
BEGIN
    ...
    -- Add teacher_id if missing (consistent with bookings/class_logs)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'teacher_id') THEN
        ALTER TABLE public.appointments ADD COLUMN teacher_id UUID REFERENCES public.profiles(id);
    END IF;

    -- Migration: If professor_id exists, move data to teacher_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'professor_id') THEN
        UPDATE public.appointments SET teacher_id = professor_id WHERE teacher_id IS NULL;
        -- ALTER TABLE public.appointments DROP COLUMN professor_id; -- Optional: Keep for safety or drop
    END IF;
END $$;

-- 2. Class Logs Table: Add missing columns
DO $$ 
BEGIN
...
-- 3. RLS Policies for Appointments (Standardized names)
