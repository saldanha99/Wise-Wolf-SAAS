...
const outputDirectory = commercialUseAllowed
  ? path.join(projectRoot, 'public/assets/hub/videos/social')
  : path.join(projectRoot, 'remotion/previews/assets/hub/videos/social');
...
const temporaryDirectory = path.join(projectRoot, 'remotion/.renders/stories');
...
if (!commercialUseAllowed) {
  console.warn(previewOnly
    ? 'Modo de prévia Story forçado: nenhum arquivo público ou receipt será alterado.'
    : 'Locução sem licença comercial validada: os Stories serão renderizados somente em remotion/previews.');
}
...
for (const content of HUB_VIDEOS) {
  ...
  const compositionId = `${content.id}Story`;
  ...
  if (commercialUseAllowed) {
    if (!compositionSourceSha256 || !remotionVersion) throw new Error('Proveniência comercial dos Stories não foi inicializada.');
    ...
    if (!forceRender) {
      ...
      if (!validation.valid) console.warn(`Cache público Story rejeitado para ${content.slug}: ${validation.reason || 'receipt inválido'}.`);
    }
  } else {
    try {
  ...
  if (reuseVideo) {
    console.log(`Reutilizando ${commercialUseAllowed ? 'pacote Story comercial validado' : 'render Story de prévia'}: ${content.slug}.mp4`);
  } else {
    ...
    console.log(
      `Masterização Story: ${content.slug} ${mastering.after.integratedLufs.toFixed(1)} LUFS / ${mastering.after.truePeakDbtp.toFixed(1)} dBTP`,
    );
  }
  ...
  if (commercialUseAllowed && !reuseVideo) {
    if (!commercialFingerprint || !compositionSourceSha256) throw new Error('Fingerprint comercial Story não foi criado.');
    ...
    if (sourceHashAfterRender !== compositionSourceSha256) {
      throw new Error(`As fontes Remotion mudaram durante o render Story de ${content.slug}; nenhum receipt foi emitido.`);
    }
  ...
  console.log(`Concluído: social/${content.slug}.mp4 (${(outputStats.size / 1024 / 1024).toFixed(1)} MB)`);
}
...
const websiteManifest = {
  ...
  videos: Object.fromEntries(HUB_VIDEOS.map((content) => [content.slug, {
    video: `/assets/hub/videos/social/${content.slug}.mp4`,
    poster: `/assets/hub/videos/social/posters/${content.slug}.webp`,
    captions: `/assets/hub/videos/social/captions/${content.slug}.pt-BR.vtt`,
    compositionId: `${content.id}Story`,
    durationSeconds: voiceManifest[content.slug].durationSeconds,
    language: 'pt-BR',
    ...(commercialUseAllowed ? {
      receipt: `/assets/hub/videos/social/receipts/${content.slug}.json`,
...
console.log('Coleção vertical Wise Wolf Hub concluída.');
