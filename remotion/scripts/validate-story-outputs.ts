...
if (modeArgument.length !== 1 || !['--preview', '--public'].includes(modeArgument[0])) {
  console.error('Uso: npm run video:validate:stories -- --preview | --public');
  process.exit(2);
}
...
if (validationMode === 'public' && !HUB_VIDEOS.every((content) => voiceManifest[content.slug]?.commercialUseAllowed === true)) {
  failures.push('coleção Story pública: há locuções sem licença comercial registrada');
}
...
for (const content of HUB_VIDEOS) {
  ...
  try {
    ...
    if (validationMode === 'public') {
      if (!compositionSourceSha256 || !remotionVersion) throw new Error('proveniência Story pública não foi inicializada');
      ...
      if (!receiptValidation.valid) failures.push(`${content.slug}: receipt comercial Story inválido (${receiptValidation.reason || 'motivo desconhecido'})`);
    }
    ...
    if (videoStream?.width !== VIDEO_STORY_WIDTH || videoStream?.height !== VIDEO_STORY_HEIGHT) failures.push(`${content.slug}: resolução Story inesperada`);
    ...
    if (mp4Stats.size > 36 * 1024 * 1024) failures.push(`${content.slug}: arquivo Story supera 36 MB`);
    if (posterStats.size < 10 * 1024) failures.push(`${content.slug}: pôster Story parece inválido`);
    ...
    console.log(`${content.slug.padEnd(14)} ${duration.toFixed(1).padStart(5)}s  ${(mp4Stats.size / 1024 / 1024).toFixed(1).padStart(5)} MB  H.264/AAC  1080x1920${masteringSummary}`);
  } catch (error) {
...
if (failures.length > 0) {
  ...
  console.log(`Todos os Stories, pôsteres e arquivos de legenda do modo ${validationMode} foram validados.`);
}
