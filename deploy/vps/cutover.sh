...
if [ "$RUN" = "yes" ]; then
  ...
  docker exec -i supabase-db psql -U supabase_admin -d postgres -v ON_ERROR_STOP=1 \
    < /opt/wisewolf/migration/post-restore-hardening.sql
...
if [ "$RUN" = "yes" ]; then
  ...
  PAYLOAD="$(jq -nc --arg url "$INBOUND_URL" '{webhook:{enabled:true,url:$url,byEvents:false,base64:false,events:["MESSAGES_UPSERT"]}}')"
