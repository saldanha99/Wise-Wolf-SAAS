...
if [[ "$VITE_HUB_PUBLIC_VIDEOS" = "true" && "${SKIP_HUB_VIDEO_VALIDATE:-false}" != "true" ]]; then
...
ssh -o BatchMode=yes "$DEPLOY_SSH_HOST" bash -s -- \
  ...
  "$DEPLOY_RELEASES_DIR" "$DEPLOY_BACKUPS_DIR" "$DEPLOY_PUBLIC_URL" <<'REMOTE'
set -Eeuo pipefail
expected_host=$1
expected_user=$2
app_dir=$3
...
for shared_name in automation-auth.ts invite-registration.ts opportunity-dispatch.ts payment-auth.ts enrollment-progress.ts asaas-creation-guard.ts asaas-capability-fence.ts asaas-mutation-guard.ts asaas-subscription-mutation.ts student-billing-period-guard.ts student-provider-lifecycle.ts saas-owner-activation.ts tenant-communication.ts tenant-legal-assets.ts tenant-integration-broker.ts hub-provider-operations.ts financial-report-message-fence.ts; do
