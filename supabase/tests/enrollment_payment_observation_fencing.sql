...
select pg_temp.assert_true(
  ...
  and (select payload ->> 'processing_state' = 'COMPLETED'
     from observation_results where label = 'activation-settled'),
  'subscription activation did not resolve and complete exact offer'
);
...
select pg_temp.assert_true(
  (select payload ->> 'error' = 'PAYMENT_BINDING_CHANGED'
  ...
  and (select payload ->> 'error' = 'PAYMENT_BINDING_CHANGED'
     from observation_results where label = 'cross-offer-profile-fallback'),
  'offer completion consumed another offer fee binding'
);
