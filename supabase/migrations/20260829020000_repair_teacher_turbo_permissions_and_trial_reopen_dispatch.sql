-- Mantém permissões de RPC do turbo consistentes para produção/ambiente parcial e
-- corrige o fluxo de reabertura de experimental para permitir redisparo
-- genérico quando a oportunidade estava direcionada.

-- 1) Reforça privilégios explícitos dos RPCs de Turbo para o que o frontend
-- realmente usa.
DO $$
BEGIN
...
-- 2) Quando uma experimental é reaberta pelo fluxo de broadcast, elimina o estado
-- direcionado legado (AWAITING_TEACHER/AWAITING_STUDENT) para permitir que ela
-- volte a entrar no fluxo de divulgação genérico.
...
begin
  ...
  if v_opportunity.status = 'OPEN'
    ...
    -- IDEMPOTÊNCIA: se os dados não mudaram, mantém o vínculo de oferta e
    -- não força cancelamento extra no vínculo anterior.
  ...
  -- Oportunidade com direção ativa pode ter sido direcionada via link/tentativa.
  -- Antes de reabrir, encerra os vínculos de aceite pontual para voltar ao
  -- broadcast padrão (todos os professores aptos).
