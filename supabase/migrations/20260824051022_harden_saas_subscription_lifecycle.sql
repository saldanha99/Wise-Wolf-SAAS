-- Assinaturas pagas do School OS falham de forma fechada quando a cobranca
-- expira ou e revertida. Somente tenants sem qualquer checkout vinculado
-- preservam o status administrativo legado ate a migracao do ciclo pago.

CREATE TABLE IF NOT EXISTS public.saas_billing_event_inbox (
...
CREATE INDEX IF NOT EXISTS saas_billing_event_inbox_retry_idx
  ON public.saas_billing_event_inbox(status, lease_expires_at)
  WHERE status IN ('PROCESSING', 'FAILED');
CREATE INDEX IF NOT EXISTS saas_billing_event_inbox_checkout_idx
  ON public.saas_billing_event_inbox(checkout_id, created_at DESC);

ALTER TABLE public.saas_billing_event_inbox ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.saas_billing_event_inbox
  FROM PUBLIC, anon, authenticated;
GRANT ALL ON TABLE public.saas_billing_event_inbox TO service_role;
  ...
  SELECT EXISTS (
    ...
    WHERE tenant.id = p_tenant_id
      ...
      AND (
        NOT EXISTS (
          SELECT 1
          FROM public.saas_checkout_intents AS paid_checkout
          WHERE paid_checkout.tenant_id = tenant.id
        )
        OR (
          tenant.current_period_end IS NOT NULL
          AND tenant.current_period_end > pg_catalog.now()
          AND EXISTS (
            SELECT 1
            FROM public.saas_checkout_intents AS live_checkout
            WHERE live_checkout.tenant_id = tenant.id
              AND live_checkout.provisioned_at IS NOT NULL
              AND live_checkout.status = 'PROVISIONED'
          )
        )
      )
  );
