...
serve(async (req) => {
  ...
  try {
    ...
    const S = auth.context.admin;
    const caller = auth.context.profile;
    if (!caller?.tenant_id && caller?.role !== "SUPER_ADMIN") {
      return json({ error: "tenant required" }, 403);
    }
    ...
    if (action === "createAbsence") {
      ...
      if (!teacherId || !startsAt || !endsAt) {
        return json({ error: "params invalidos" }, 400);
      }

      const { data: absence, error: aerr } = await S.from("teacher_absences")
        .insert({
      ...
      const { data: bookings } = await S.from("bookings")
        .select(
          "id, student_id, day_of_week, time_slot, profiles!bookings_student_id_fkey(full_name)",
        )
      ...
      for (const b of (bookings || [])) {
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
          if (d.getDay() === dow) {
            const prof = Array.isArray((b as any).profiles)
              ? (b as any).profiles[0]
              : (b as any).profiles;
            classes.push({
      ...
      classes.sort((a, b) =>
        (a.classDate + a.classTime).localeCompare(b.classDate + b.classTime)
      );
    ...
    if (action === "requestCoverage") {
      ...
      if (!absenceId || !Array.isArray(items) || !items.length) {
        return json({ error: "params invalidos" }, 400);
      }
      const { data: absence } = await S.from("teacher_absences").select("*").eq(
        "id",
        absenceId,
      ).maybeSingle();
      if (
        !absence ||
        (absence.tenant_id !== caller.tenant_id &&
          caller.role !== "SUPER_ADMIN")
      ) return json({ error: "absence invalida" }, 404);

      // instância central p/ WhatsApp
      let instance: string | null = null;
      if (mode !== "force") {
        instance = await loadTenantCentralWhatsAppInstance(
          S,
          absence.tenant_id,
        );
      }
      ...
      for (const it of items) {
        ...
        const { data: cov, error: cerr } = await S.from("class_coverages")
          .insert({
            original_teacher_id: absence.teacher_id,
            cover_teacher_id: it.coverTeacherId,
            student_id: it.studentId,
            booking_id: it.bookingId,
            absence_id: absenceId,
            tenant_id: caller.tenant_id,
            class_date: it.classDate,
            class_time: it.classTime,
        ...
        if (cerr) {
          out.push({ item: it, error: cerr.message });
          continue;
        }

        // dispara WhatsApp de aceite ao substituto
        if (!isForce && instance && token) {
          const { data: cover } = await S.from("profiles").select(
            "full_name, phone",
          ).eq("id", it.coverTeacherId).maybeSingle();
          let phone = (cover?.phone || "").replace(/\D/g, "");
          if (phone.length === 10 || phone.length === 11) phone = "55" + phone;
          if (phone.length >= 12) {
            const link =
              `${SUPABASE_URL}/functions/v1/accept-coverage?token=${token}`;
            const nome = (cover?.full_name || "").split(" ")[0];
            const dataFmt = new Date(it.classDate + "T00:00:00")
            ...
            try {
              ...
              /* envio é melhor-esforço; a cobertura fica pendente no painel */
            }
          }
        }
    ...
    if (action === "listCoverages") {
      ...
      const { data } = await S.from("class_coverages").select("*").eq(
        "absence_id",
        absenceId,
      ).order("class_date");
