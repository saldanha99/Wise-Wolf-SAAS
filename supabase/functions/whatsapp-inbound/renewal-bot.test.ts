...
Deno.test("aluno pede horários: abre negociação e pergunta ao professor atual", async () => {
  const { deps, calls, sent } = fakeDeps({
    ...
    open_renewal_negotiation: {
      ok: true,
      action: "ask_teacher",
      teacher_name: "Teacher Sintética",
      teacher_phone: "5511900000001",
      reply_code: "A1B2C3D4",
      slots: [{ day: "Segunda", time: "14:00" }, {
        day: "Terça",
        time: "14:30",
  ...
  const handled = await handleRenewalStudentMessage(
    deps,
    student,
    "5511900000009",
    "quero 3x: segunda as 14h e terça e sexta as 14:30",
    "msg-1",
  );
...
Deno.test("aluno sem renovação aberta não é capturado", async () => {
  ...
  assertEquals(
    await handleRenewalStudentMessage(deps, student, "55119", "oi", "msg-2"),
    false,
  );
});
...
Deno.test("frequência sem dias pede os dias antes de abrir negociação", async () => {
  ...
  await handleRenewalStudentMessage(
    ...
    student,
    "5511900000009",
    "quero 3 vezes por semana",
    "msg-3",
  );
  assertEquals(
    calls.some((call) => call.fn === "open_renewal_negotiation"),
    false,
  );
...
Deno.test("professor aceita: Gestão recebe o pedido de aprovação com código", async () => {
  const { deps, sent } = fakeDeps({
    ...
    respond_renewal_teacher_request: {
      ok: true,
      action: "await_management",
      approval_code: "C0FFEE11",
      student_name: "Aluna Sintética",
      student_phone: "5511900000009",
      teacher_name: "Teacher Sintética",
      classes_per_week: 3,
      slots: [{ day: "Segunda", time: "14:00" }],
      suggested_fee_cents: 26100,
    },
  });
  const handled = await handleRenewalTeacherReply(
    deps,
    { id: "teacher-1", name: "Teacher Sintética" },
    "5511900000001",
    "sim #A1B2C3D4",
    "msg-5",
  );
  assertEquals(handled, true);
  const group = sent.find((message) =>
...
Deno.test("Gestão aprova com valor e o grupo recebe a confirmação", async () => {
  ...
  const handled = await handleRenewalManagementCommand(
    deps,
    "admin-1",
    "120363400000000099@g.us",
    "aprovar #C0FFEE11 261",
  );
  assertEquals(handled, true);
