...
function buildWebhookEnrollmentObservation(input: {
  ...
  return {
    ...
    providerCustomerId,
    providerSubscriptionId: String(input.payment.subscription || "").trim() ||
      null,
  ...
  try {
    ...
    if (settledPayment || enrollmentPaymentUnsettled) {
      const profileQuery = supabase
        .from("profiles")
        .select(
          "role,tenant_id,phone,full_name,enrollment_payment_id,subscription_id,is_test_account",
        )
        .eq("id", studentId)
      ...
      if (settledPayment) {
        ...
        if (
          ...
          const studentName =
            safeCommunicationText(profileData.full_name?.split(" ")[0], 80) ||
            "Aluno";
          ...
          }!*\nObrigado, ${studentName}. O recebimento foi registrado.`;
          ...
          const { error: queueError } = await supabase
            ...
            .insert({
              ...
              student_phone: profileData.phone,
