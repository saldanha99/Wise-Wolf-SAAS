...
const ASAAS_ACCESS_TOKEN = Deno.env.get('ASAAS_ACCESS_TOKEN') || Deno.env.get('ASAAS_API_KEY');
...
serve(async (req) => {
    ...
    try {
        ...
        // Validation check (Optional weak check)
        const requestToken = req.headers.get('asaas-access-token');
        if (ASAAS_ACCESS_TOKEN && requestToken !== ASAAS_ACCESS_TOKEN) {
            console.warn(`[Webhook] Token Mismatch! Expected: ${ASAAS_ACCESS_TOKEN.slice(0, 5)}...`);
...
Prepare-se, as suas aulas de ${freq}x por semana começam em breve!`;
                ...
                // C. Sincronização com Caixa da Escola (Financial Transactions)
                // NOW HANDLED BY DATABASE TRIGGER 'trg_sync_financial'
                // We keep this here just in case, but the Trigger is the Source of Truth.
                // Or better, let's remove it to avoid conflicts? 
                // Let's keep it as fallback OR trust the trigger. 
                // Given I created the trigger, I can rely on it. 
                // But debugging the trigger might be broken if 'tenant_id' is missing.

                // For now, logging.
                console.log('ℹ️ Financial Transaction should be handled by DB Trigger trg_sync_financial.');
            }
