    ...
    try {
        ...
        */
        ...
        if (payment.externalReference && isValidUUID(payment.externalReference)) {
            // Check if this UUID is a Prospect instead of a Student
            const { data: prospectCheck } = await supabase
                .from('prospects')
                .select('id, promoted_at, promoted_to_id')
                .eq('id', payment.externalReference)
                .maybeSingle();

            if (prospectCheck && !prospectCheck.promoted_at) {
                console.log(`🚀 Payment is for an unpromoted Prospect: ${prospectCheck.id}. Calling complete-enrollment...`);
                // Trigger complete-enrollment only on confirmation
                if (event === 'PAYMENT_RECEIVED' || event === 'PAYMENT_CONFIRMED') {
                    const supabaseUrl = Deno.env.get('SUPABASE_URL');
                    try {
                        const callRes = await fetch(`${supabaseUrl}/functions/v1/complete-enrollment`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'x-webhook-secret': ASAAS_WEBHOOK_TOKEN
                            },
                            body: JSON.stringify({
                                prospect_id: prospectCheck.id,
                        ...
                        if (callRes.ok) {
                            ...
                            studentId = callData.profile_id;
                            console.log(`✅ Prospect promoted to Student ID: ${studentId}`);
                        } else {
                            console.error('❌ Failed to promote prospect:', await callRes.text());
                        }
                    } catch (e) {
                        console.error('❌ Exception promoting prospect:', e);
                    }
                } else {
                    console.log(`ℹ️ Event ${event} is not a confirmation. Skipping promotion.`);
                }
            } else if (prospectCheck && prospectCheck.promoted_at) {
                // If it was already promoted, use the actual student ID
                studentId = prospectCheck.promoted_to_id;
            } else {
                // Standard student ID fallback
        ...
        if (event === 'PAYMENT_RECEIVED' || event === 'PAYMENT_CONFIRMED' || event === 'PAYMENT_UPDATED') {
            ...
            if (studentId) {
                ...
                const { data: profileData, error: profileFetchErr } = await supabase
                    .from('profiles')
                    .select('tenant_id, contract_accepted, welcome_sent_at, phone, full_name, signed_document_url, class_frequency, referrer_teacher_id, referrer_student_id')
                    .eq('id', studentId)
                    .single();
                ...
                const isEnrollment = payment.description && payment.description.toLowerCase().includes('matrícula');
                
                const profileUpdatePayload: any = { status_financial: 'ACTIVE' };
                if (isEnrollment) {
                    profileUpdatePayload.enrollment_fee_paid = true;
                    profileUpdatePayload.enrollment_payment_id = payment.id;
                }
...
Prepare-se, as suas aulas de ${freq}x por semana começam em breve!`;
                ...
                // If this student was referred, check if reward should be released
                if (event === 'PAYMENT_CONFIRMED' || event === 'PAYMENT_RECEIVED') {
                    try {
                        const referrerTeacherId = profileData?.referrer_teacher_id;
                        const referrerStudentId = profileData?.referrer_student_id;
                        const referrerId = referrerTeacherId || referrerStudentId;

                        if (referrerId) {
                            console.log(`🎁 [Affiliate] Checking reward for referrer: ${referrerId}`);
                            ...
                            const { data: existingConv } = await supabase
                                ...
                                .eq('tenant_id', paymentData.tenant_id || 'school-wise-wolf')
                                .maybeSingle();

                            if (existingConv) {
                                // Update to PAYMENT_CONFIRMED if still pending
                                if (existingConv.status === 'PENDING_PAYMENT') {
                                    await supabase
                                        .from('affiliate_conversions')
                                        .update({
                                    ...
                                    // Trigger reward release
                                    const { data: rewardResult } = await supabase
                                        .rpc('release_affiliate_reward', { p_conversion_id: existingConv.id });

                                    console.log('🎁 [Affiliate] Reward result:', JSON.stringify(rewardResult));
                                }
                            } else {
                                // Find affiliate code for the referrer
                                ...
                                if (affCode) {
                                    // Create conversion + trigger reward
                                    const { data: newConv, error: convErr } = await supabase
                                        ...
                                        .insert({
                                            ...
                                            tenant_id: paymentData.tenant_id || 'school-wise-wolf',
                                            status: 'PAYMENT_CONFIRMED',
                                            payment_id: payment.id
                                        })
                                        .select('id')
                                    ...
                                    if (!convErr && newConv) {
                                        const { data: rewardResult } = await supabase
                                            .rpc('release_affiliate_reward', { p_conversion_id: newConv.id });

                                        console.log('🎁 [Affiliate] New conversion + reward:', JSON.stringify(rewardResult));
                                    }
                                } else {
                                    console.log('ℹ️ [Affiliate] Referrer has no active affiliate code, skipping.');
                                }
                            }
                        }
                    } catch (affErr) {
                        // Non-blocking: don't fail webhook because of affiliate logic
                        console.error('⚠️ [Affiliate] Error processing reward (non-blocking):', affErr);
                    }
                }
                }
