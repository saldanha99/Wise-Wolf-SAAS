...
serve(async (req) => {
  ...
  try {
    ...
    const startMonth = offer
      ...
      : text(body.startDate);
    const proRata = offer
      ? Boolean(offerPayload.enableProRata)
      : Boolean(body.proRata);
    ...
    const rawAsaasCustomerId = text(profile.asaas_customer_id);
    ...
    const customerQueries: Array<Record<string, string>> = [
      { externalReference: userId, includeDeleted: "true" },
    ...
    const buildCreationCandidate = (anchor: Date) => {
      ...
      const recurringNextDueDate = isOneTime
        ? ""
