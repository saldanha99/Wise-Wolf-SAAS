...
serve(async (req) => {
    ...
    try {
        ...
        const { name, email, phone, hourlyRate, pixKey, zoomLink, whatsappId, tenantId, contractUrl } = await req.json()
        // Default to 'school-wise-wolf' if not provided for backward compatibility, but log warning
        const finalTenantId = tenantId || 'school-wise-wolf';
        ...
        // 1. Create Auth User
        console.log(`Creating auth user for ${email} in tenant ${finalTenantId}...`);
        const { data: authData, error: authError } = await supabaseClient.auth.admin.createUser({
            email: email,
            password: '123456', // Hardcoded default password as requested
            email_confirm: true,
            user_metadata: { full_name: name, role: 'TEACHER', tenant_id: finalTenantId }
        })
