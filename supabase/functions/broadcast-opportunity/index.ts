...
const BASE_URL = "http://localhost:3000/claim-opportunity";
...
serve(async (req) => {
    ...
    try {
        ...
        const { student_name, student_phone, date, time, interests } = await req.json();
        ...
        const textMessage = `🐺 *OPORTUNIDADE RELÂMPAGO!*

Aluno: ${student_name}
Data: ${formattedDate} (${dayString}) às ${time}
Objetivo: ${interests || 'Não informado'}

👇 *Quem clicar primeiro leva:*
${claimLink}`;
