...
serve(async (req) => {
  ...
  try {
    ...
    const integration = await resolveAsaasIntegration(
      authorization.admin,
      authorization.tenantId,
      profileCustomerIdAtAuthorization ? "customer.read" : "customer.create",
    );
    let asaasCustomerId = profileCustomerIdAtAuthorization || null;
    ...
    if (offer) {
      ...
      profileUpdate.attendance_phone = isDependent
        ? digits(offerPayload.studentPhone) || null
        : null;
    ...
    if (schedule.length > 0 && professorId) {
      ...
      const rows = schedule.flatMap((rawSlot) => {
        ...
        return [{
          ...
          time_slot: time,
          start_date: offer
            ? text(offerPayload.startDate) ||
              new Date().toISOString().slice(0, 10)
            : text(body.startDate) || new Date().toISOString().slice(0, 10),
        }];
      });
