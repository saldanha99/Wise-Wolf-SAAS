...
serve(async (req) => {
  ...
  const studentLevel = ALLOWED_LEVELS.has(requestedLevel)
    ? requestedLevel
    : "B1";
  ...
  const [profileRes, wolfIntelRes] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "accumulated_context, weak_points, strong_points, recommended_approach",
      )
  ...
  const buildSystemPrompt = (): string => {
    ...
    if (profile?.english_for) {
      memLines.push(`- Learning English for: ${safe(profile.english_for)}`);
    }
    if (profile?.short_term_goal) {
      memLines.push(`- Short-term goal: ${safe(profile.short_term_goal)}`);
    }
    if (wolfIntel?.strong_points?.length) {
      memLines.push(
        `- Already strong at: ${
          safe(wolfIntel.strong_points.slice(0, 3).join(", "))
        }`,
      );
    }
    if (wolfIntel?.weak_points?.length) {
      memLines.push(
        `- Still struggles with: ${
          safe(wolfIntel.weak_points.slice(0, 3).join(", "))
        }`,
      );
    }
    if (wolfIntel?.accumulated_context) {
      memLines.push(
        `- Background: ${safe(wolfIntel.accumulated_context, 800)}`,
      );
    }
    if (wolfIntel?.recommended_approach) {
      memLines.push(
        `- Teaching approach: ${safe(wolfIntel.recommended_approach)}`,
      );
    }
...
- NEVER use markdown, asterisks, bullet points, or any formatting. Speak as if talking aloud`;
  ...
  clientWs.onopen = () => {
    ...
    geminiWs.onclose = (ev) => {
      ...
      sendToClient({
        type: "error",
        message: "Conexão com a IA foi encerrada.",
      });
