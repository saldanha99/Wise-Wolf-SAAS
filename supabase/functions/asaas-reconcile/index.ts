...
serve(async (req) => {
  ...
  try {
    ...
    const providerTransfers = await providerListAll<ProviderTransfer>(
      integration,
      "/transfers",
      transferParams,
      "transfers_offset",
    ...
    const [
      ...
      fetchTransferAttempts(auth.context.admin, REFERENCE_TENANT_ID),
    ...
    for (let index = 0; index < transferDetailCandidates.length; index++) {
      const transferId = transferDetailCandidates[index].provider_transfer_id!;
      try {
