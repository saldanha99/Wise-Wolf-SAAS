...
const EVOLUTION_API_BASE = `${
  (Deno.env.get("EVOLUTION_API_URL") || "https://api.2b.app.br").replace(
    /\/+$/,
    "",
  )
}/message/sendText`;
...
const DEFAULT_TRAINING: Record<Role, string> = {
  secretaria:
    'Você é a gestora da equipe de IA da escola. Junte os relatórios da atendente, da coordenação pedagógica e do financeiro e escreva um briefing curto e direto para o diretor, começando com "Bom dia". Destaque o que é urgente. Não invente dados.',
  atendente:
...
async function buildAtendente(
  ...
  try {
    ...
    const done = actionable.filter((lead: any) =>
      String(lead.status).toUpperCase() === "TRIAL_DONE"
    );
    if (done.length) {
      ...
      lines.push(
        `- 🎯 **${done.length} aluno(s) fizeram a experimental e ainda não têm matrícula confirmada** — acompanhar conversão: ${
          done.slice(0, 6).map((l: any) => l.name || "—").join(", ")
        }`,
      );
    }
...
async function runForTenant(
  ...
  const joined = reports.map((r) =>
    `### ${ROLE_META[r.role as Role].emoji} ${
      ROLE_META[r.role as Role].label
    } (${r.name})\n${r.markdown}`
  ).join("\n\n");
  const secBase =
    `Bom dia! Sou **${sec.name}**, sua secretária. Resumo da equipe hoje (${dateStr}):\n\n${joined}`;
  const secMd = useAi
    ? await refine("secretaria", sec.name, sec.training, secBase)
    : secBase;
  const secretary = {
    role: "secretaria",
    name: sec.name,
  ...
  try {
    ...
    if (mode === "send") {
      ...
      for (const t of (tenants || [])) {
        const tenantAccess = await loadOperationalTenantAccess(admin, t.id);
        if (!tenantAccess.ok) {
          result.failures.push(`${t.id}: status operacional indisponível`);
          continue;
        }
        ...
        if (!instance || phone.length < 12) {
          result.failures.push(`${t.id}: sem instância central ou telefone`);
          continue;
        }
        try {
          const { secretary } = await runForTenant(admin, t.id, cfg, useAi);
          const resp = await sendWhats(instance, {
            number: phone,
            text: wa(secretary.markdown),
            delay: 800,
            linkPreview: false,
          });
          if (!resp.ok) {
            result.failures.push(`${t.id}: evolution ${resp.status}`);
            continue;
          }
          await admin.from("automation_sent").insert({
            kind: "SCHOOL_AI_BRIEFING",
            subject_id: t.id,
            ref_date: todayBRT,
          });
          result.sent++;
    ...
    const me = requestAuth.context.profile;
    if (!me?.tenant_id) return json({ error: "tenant_obrigatorio" }, 403);

    const { data: tenant } = await admin.from("tenants").select(
      "ai_team_config",
