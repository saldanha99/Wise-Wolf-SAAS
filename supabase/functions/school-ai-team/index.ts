...
//                          ai_team_config.schedule='daily' e enfileira o briefing ao diretor.
// A fila fecha a corrida entre dois crons e preserva o estado quando o provedor cai.
...
const ROLE_META: Record<
  Role,
  { label: string; defaultName: string; emoji: string }
> = {
...
const DEFAULT_TRAINING: Record<Role, string> = {
  secretaria:
    'Você é a gestora da equipe de IA da escola. Entregue um briefing curto e estratégico para o diretor, começando com "Bom dia". Destaque o que é urgente e proponha as próximas ações objetivas.',
  atendente:
    "Cuide do relacionamento e do comercial: leads novos sem contato, alunos que fizeram a aula experimental e ainda não matricularam, aniversariantes do dia e alunos sumidos para resgate. Tom cordial e prático.",
  estagiario:
    "Acompanhe a operação pedagógica: aulas não lançadas/atrasadas por professor, reposições pendentes, aulas experimentais paradas e transferências de professor a aplicar. Seja objetivo e priorize por urgência.",
  financeiro:
    "Aja como gerente financeiro da escola: inadimplência (mensalidades vencidas), valores a receber a vencer, fechamentos de professores a pagar e a saúde do caixa do mês. Recomende o que cobrar/pagar primeiro.",
  rh:
    "Acompanhe candidaturas, triagens pendentes e entrevistas a agendar. Não prometa contratação e não altere decisões do diretor.",
};
...
function defaultConfig(): TeamConfig {
  ...
  (Object.keys(ROLE_META) as Role[]).forEach((r) => {
    agents[r] = {
      name: ROLE_META[r].defaultName,
      enabled: true,
      training: DEFAULT_TRAINING[r],
    };
  });
...
function rateRefineQuality(text: string, role: Role) {
  ...
  const urgentScore =
    /urgente|prioridade|ação|pendente|cobrar|hoje|agora|decidir/i.test(clean)
      ? 1
      : 0.2;
  const structureScore =
    /(?:Resumo|Prioridade|Ação|Alerta|Recomend)/i.test(clean) ? 1 : 0.4;
  let score = (lengthScore * 0.35) + (bulletScore * 0.25) +
    (urgentScore * 0.2) + (structureScore * 0.2);
  if (
    role === "secretaria" && !/^(bom dia|boa tarde|boa noite)!?/i.test(clean)
  ) score -= 0.12;
...
async function buildEstagiario(
  sb: any,
  tenantId: string,
): Promise<AgentReport> {
  const hl: string[] = [];
  ...
  try {
    ...
    const [{ data: bookings }, { data: logs }, { data: teachers }] =
      await Promise.all([
        sb.from("bookings").select("id, day_of_week, start_date, teacher_id")
          .eq("tenant_id", tenantId).not("day_of_week", "is", null),
        sb.from("class_logs").select("booking_id, class_date").eq(
          "tenant_id",
          tenantId,
        ).gte("class_date", startStr),
        sb.from("profiles").select("id, full_name").eq("tenant_id", tenantId)
          .in("role", ["TEACHER", "teacher"]),
      ]);
    const nameById: Record<string, string> = {};
    (teachers || []).forEach((t: any) => {
      nameById[t.id] = t.full_name;
    });
    const loggedBooking = new Set(
      (logs || []).map((l: any) => `${l.booking_id}|${l.class_date}`),
    );
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      for (const b of (bookings || [])) {
        ...
        if (!loggedBooking.has(`${b.id}|${dateStr}`)) {
          pendingByTeacher[b.teacher_id] =
            (pendingByTeacher[b.teacher_id] || 0) + 1;
        }
      }
    }
    const totalPend = Object.values(pendingByTeacher).reduce(
      (s, n) => s + n,
      0,
    );
    if (totalPend > 0) {
      const priority: Priority = totalPend > 14
        ? "urgente"
        : totalPend > 7
        ? "alta"
        : "media";
      ...
      addPriority(
        priorities,
        "estagiario",
        `${totalPend} aula(s) sem lançar nos últimos 7 dias`,
        priority,
      );
      const top = Object.entries(pendingByTeacher).sort((a, b) => b[1] - a[1])
        .slice(0, 6)
        .map(([tid, n]) => `${nameById[tid] || "Professor"} (${n})`).join(", ");
      lines.push(
        `- 📝 **${totalPend} aula(s) não lançada(s) (últimos 7 dias)** — por professor: ${top}`,
      );
    }

    const { data: repos } = await sb.from("reschedules").select("id").eq(
      "tenant_id",
      tenantId,
    ).eq("date", "Pendente");
    if ((repos || []).length) {
      ...
      addPriority(
        priorities,
        "estagiario",
        `${repos.length} reposição(ões) pendente(s) de reagendamento`,
        priority,
      );
      lines.push(
        `- 🔁 **${repos.length} reposição(ões) pendente(s)** de reagendamento.`,
    ...
    ).eq("tenant_id", tenantId).eq("status", "OPEN").lt(
      "created_at",
    ...
    if ((opps || []).length) {
      ...
      addPriority(
        priorities,
        "estagiario",
        `${opps.length} aula experimental(is) aberta(s) há 3+ dias`,
        priority,
      );
      lines.push(
        `- ⏳ **${opps.length} aula(s) experimental(is) aberta(s) há 3+ dias**: ${
          (opps || []).slice(0, 5).map((o: any) => o.student_name || "—").join(
            ", ",
          )
        }`,
      );
    ...
    const { data: transfers } = await sb.from("teacher_transfers").select("id")
      .eq("tenant_id", tenantId).in("status", ["PENDING", "ACCEPTED"]);
    if ((transfers || []).length) {
      addPriority(
        priorities,
        "estagiario",
        `${transfers.length} transferência(s) de professor aguardando aceite/aplicação`,
        "baixa",
      );
      lines.push(
        `- 🔀 **${transfers.length} transferência(s) de professor** aguardando aceite/aplicação.`,
    ...
    lines.push(`- (parcial: ${(e as Error).message})`);
  }
  if (lines.length === 2) {
    lines.push("- ✅ Operação em dia: nada parado ou pendente.");
  }
...
async function buildFinanceiro(
  ...
  try {
    ...
    const { data: pays } = await sb.from("student_payments").select(
      "value, due_date, status, student_id",
    ).eq("tenant_id", tenantId).in("status", ["PENDING", "OVERDUE"]);
    const overdue = (pays || []).filter((p: any) =>
      p.due_date && p.due_date < today
    );
    const dueSoon = (pays || []).filter((p: any) =>
      p.due_date && p.due_date >= today && p.due_date <= soon
    );
    const overSum = overdue.reduce(
    ...
    if (overdue.length) {
      ...
      addPriority(
        priorities,
        "financeiro",
        `Inadimplência: ${overdue.length} mensalidade(s) vencidas (${
          fmtBRL(overSum)
        })`,
        priority,
      );
      lines.push(
        `- 🔴 **Inadimplência: ${overdue.length} mensalidade(s) vencida(s)** (${
          fmtBRL(overSum)
        }) — cobrar.`,
      );
    }
    if (dueSoon.length) {
      const v = dueSoon.reduce(
        (s: number, p: any) => s + Number(p.value || 0),
        0,
      );
      if (v > 3500) {
        addPriority(
          priorities,
          "financeiro",
          `Pagamentos a vencer em 7 dias: ${fmtBRL(v)}`,
          "media",
        );
      }
      lines.push(
        `- 🟡 **${dueSoon.length} a vencer em 7 dias** (${fmtBRL(v)}).`,
      );
    }

    const { data: closings } = await sb.from("teacher_closings").select(
      "total_amount, status",
    ...
    if ((closings || []).length) {
      const v = (closings || []).reduce(
        (s: number, c: any) => s + Number(c.total_amount || 0),
        0,
      );
      ...
      addPriority(
        priorities,
        "financeiro",
        `Fechamentos de professores a pagar: ${closings.length} itens (R$ ${
          fmtBRL(v)
        })`,
        priority,
      );
      lines.push(
        `- 👩‍🏫 **${closings.length} fechamento(s) de professor a pagar** (${
          fmtBRL(v)
        }).`,
      );
    }
    ...
    const { data: txs } = await sb.from("financial_transactions").select(
      "type, amount",
    ).eq("tenant_id", tenantId).gte("occurred_at", monthStart);
    const entradas = (txs || []).filter((t: any) =>
      String(t.type).toUpperCase() === "ENTRADA"
    ).reduce((s: number, t: any) => s + Number(t.amount || 0), 0);
    const saidas = (txs || []).filter((t: any) =>
      String(t.type).toUpperCase() !== "ENTRADA"
    ).reduce((s: number, t: any) => s + Number(t.amount || 0), 0);
    lines.push(
      `- 💰 Caixa do mês (entradas − saídas): **${
    ...
    try {
      const { data: radar } = await sb.rpc("get_mei_radar", {
        p_tenant: tenantId,
      });
      if (radar && !radar.error) {
        const proj = Math.max(
          Number(radar.projecao_media || 0),
          Number(radar.projecao_ritmo_3m || 0),
        );
        lines.push(
          `- 📡 Radar MEI ${radar.ano}: ${fmtBRL(radar.receita_acumulada)} de ${
            fmtBRL(radar.teto)
          } (${radar.pct_teto}% do teto) — projeção do ano ${
            fmtBRL(proj)
          } (${radar.pct_projecao_teto}% do teto).`,
        );
        if (Number(radar.receita_acumulada) >= Number(radar.teto)) {
          ...
          addPriority(
            priorities,
            "financeiro",
            "Faturamento MEI já passou do teto. Risco de desenquadramento e DAS complementar",
            "urgente",
          );
          lines.push(
            "- 🚨 URGENTE: o faturamento JÁ PASSOU do teto do MEI. Até R$ 97.200 paga DAS complementar e vira ME em janeiro; acima disso o desenquadramento é RETROATIVO. Falar com o contador AGORA.",
          );
        } else if (
          Number(radar.pct_teto) >= 90 || Number(radar.pct_projecao_teto) >= 100
        ) {
          hl.push(`MEI ${radar.pct_projecao_teto}% do teto (projeção)`);
          addPriority(
            priorities,
            "financeiro",
            "Radar MEI em risco: projeção no teto. Planejar migração",
            "alta",
          );
          lines.push(
            "- ⚠️ ATENÇÃO: no ritmo atual o teto do MEI vai estourar. Planejar a migração para ME (Simples Anexo III, ~6%) com o contador ainda neste ano.",
          );
        } else if (Number(radar.pct_projecao_teto) >= 75) {
          addPriority(
            priorities,
            "financeiro",
            "Radar MEI acima de 75% na projeção. Acompanhar mensalmente",
            "media",
          );
          lines.push(
            "- 🟡 Aviso: a projeção do ano já passa de 75% do teto do MEI — acompanhar mensalmente e alinhar com o contador a eventual virada para ME.",
          );
        }
      }
    } catch { /* radar indisponível não trava o relatório */ }
    if (overdue.length || (closings || []).length) {
      lines.push(
        "",
        "**Recomendação:** priorize cobrar as mensalidades vencidas e programar os fechamentos de professores.",
      );
    }
  } catch (e) {
    lines.push(`- (parcial: ${(e as Error).message})`);
...
async function buildAtendente(sb: any, tenantId: string): Promise<AgentReport> {
  ...
  try {
    ...
    const { data: leads } = await sb.from("crm_leads")
      .select("name, phone, email, status, created_at").eq(
        "tenant_id",
        tenantId,
      );
    ...
    const novos = actionable.filter((l: any) =>
      ...
      (!l.status ||
        ["NEW", "NOVO", "LEAD"].includes(String(l.status).toUpperCase()))
    );
    if (novos.length) {
      ...
      addPriority(
        priorities,
        "atendente",
        `Novos leads sem contato em 7 dias: ${novos.length}`,
        p,
      );
      lines.push(
        `- 🆕 **${novos.length} lead(s) novo(s) (7 dias) a contatar**: ${
          novos.slice(0, 6).map((l: any) => l.name || "—").join(", ")
        }`,
      );
    }

    // Conversão usa o grafo autoritativo da experimental, não o marcador legado
    // crm_leads.TRIAL_DONE (que pode ficar stale ou casar telefones compartilhados).
    const { data: trialOpportunities, error: trialOpportunitiesError } =
      await sb
        .from("opportunities")
        .select(
    ...
    if (trialOpportunitiesError) throw trialOpportunitiesError;
    ...
    if (done.length) {
      ...
      addPriority(
        priorities,
        "atendente",
        `${done.length} aluno(s) fizeram a aula experimental e ainda não têm matrícula confirmada`,
        p,
      );
      lines.push(
        `- 🎯 **${done.length} aluno(s) fizeram a experimental e ainda não têm matrícula confirmada** — acompanhar conversão: ${
          done.slice(0, 6).map((opp: any) => opp.student_name || "—").join(", ")
        }`,
      );
    }

    // Aniversariantes do dia (alunos)
    const { data: studs } = await sb.from("profiles").select(
      "full_name, birth_date",
    ).eq("tenant_id", tenantId).in("role", ["STUDENT", "student"]).not(
      "birth_date",
      "is",
      null,
    );
    ...
    const bdays = (studs || []).filter((s: any) => {
      const b = new Date(s.birth_date + "T00:00:00");
      return b.getMonth() + 1 === mm && b.getDate() === dd;
    });
    if (bdays.length) {
      lines.push(
        `- 🎂 **${bdays.length} aniversariante(s) hoje**: ${
          bdays.map((s: any) => (s.full_name || "").split(" ")[0]).join(", ")
        } — mandar parabéns.`,
      );
    }
    ...
    if (!instance) {
      addPriority(
        priorities,
        "atendente",
        "WhatsApp central não conectado. Alertas automáticos de relacionamento parados",
        "media",
      );
      lines.push(
        "- ⚠️ WhatsApp central não conectado: os avisos automáticos estão desligados.",
      );
    }
  } catch (e) {
    lines.push(`- (parcial: ${(e as Error).message})`);
  ...
  if (lines.length === 2) {
    lines.push("- ✅ Nada pendente com alunos/leads no momento.");
  }
  return { md: lines.join("\n"), hl, priorities };
}
...
async function buildRh(sb: any, tenantId: string): Promise<AgentReport> {
  ...
  try {
    const { data: apps } = await sb.from("job_applications")
      .select(
        "name, status, ai_recommendation, preinterview_status, interview_slot",
      )
      .eq("tenant_id", tenantId)
      .order("created_at", { ascending: false })
      .limit(200);
    const open = (apps || []).filter((app: any) =>
      !["CONTRATADO", "REJEITADO"].includes(
    ...
    const preinterview = open.filter((app: any) =>
      ["SENT", "IN_PROGRESS"].includes(
        String(app.preinterview_status || "").toUpperCase(),
      )
    );
    ...
    if (unscreened.length) {
      ...
      addPriority(
        priorities,
        "rh",
        `Candidaturas sem triagem: ${unscreened.length}`,
        priority,
      );
      lines.push(
        `- 🆕 **${unscreened.length} candidatura(s) aguardam triagem**.`,
      );
    }
    if (preinterview.length) {
      lines.push(
        `- 💬 **${preinterview.length} pré-entrevista(s) em andamento**.`,
      );
    }
    if (interviews.length) {
      ...
      addPriority(
        priorities,
        "rh",
        `${interviews.length} candidato(s) recomendado(s) sem entrevista marcada`,
        "media",
      );
      lines.push(
        `- 📅 **${interviews.length} candidato(s) recomendados ainda sem entrevista marcada**: ${
          interviews.slice(0, 6).map((app: any) => app.name || "—").join(", ")
        }.`,
      );
    }
  } catch (e) {
  ...
  if (lines.length === 2) {
    lines.push("- ✅ Nenhuma pendência de recrutamento no momento.");
  }
  return { md: lines.join("\n"), hl, priorities };
}
  ...
  for (const model of MODELS) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (accepted && score >= qualityTarget) {
          return {
            markdown: candidate,
            meta: {
              usedAi: true,
        ...
        if (attempt === 1) {
          return {
            ...
            meta: {
              ...
              retries: 1,
              quality: score,
              warning: "texto abaixo do mínimo",
            },
          };
        }
...
const wa = (md: string) =>
  md.replace(/\*\*(.+?)\*\*/g, "*$1*").replace(/^### /gm, "").replace(
    /^#+ /gm,
    "",
  ).replace(/^- /gm, "• ");
...
async function runForTenant(
  ...
  for (
    const role of ["estagiario", "financeiro", "atendente", "rh"] as Role[]
  ) {
    ...
    const ref = useAi
      ? await refine(
        role,
        cfg.agents[role].name,
        cfg.agents[role].training,
        base.md,
        base.priorities,
      )
      : { markdown: base.md, meta: localRefineMeta("off") };
  ...
  const secRef = useAi
    ...
    : { markdown: secBase, meta: localRefineMeta("off") };
  ...
  try {
    ...
    if (mode === "send") {
      ...
      for (const t of (tenants || [])) {
        ...
        const { data: duplicate } = await admin.from("automation_sent").select(
          "id",
        )
          .eq("kind", "SCHOOL_AI_BRIEFING").eq("subject_id", t.id).eq(
            "ref_date",
            todayBRT,
          ).maybeSingle();
        if (duplicate) {
        ...
        if (phone.length < 12) {
          result.failures.push(`${t.id}: telefone do diretor ausente`);
          continue;
        }
        try {
          const { secretary } = await runForTenant(admin, t.id, cfg, useAi);
          const queuedAt = new Date().toISOString();
          const { error: queueError } = await admin.from("notification_queue")
            .insert({
              tenant_id: t.id,
              student_phone: phone,
              message_body: wa(secretary.markdown),
              ...
              notification_kind: "SCHOOL_AI_BRIEFING",
              source_type: "SCHOOL_AI_BRIEFING",
              class_date: todayBRT,
              idempotency_key: `school-ai-briefing:${todayBRT}`,
            });
          ...
          if (queueError) {
            result.failures.push(
              `${t.id}: fila ${queueError.code || "indisponível"}`,
            );
            continue;
          }
          result.queued++;
        } catch (e) {
          result.failures.push(`${t.id}: ${(e as Error).message}`);
        }
      }
      return json(result);
    }
