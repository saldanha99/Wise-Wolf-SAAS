  ...
  try {
    ...
    if (token) {
      ...
      try {
        ...
        if (!offerId && !oppId) {
          return new Response(
            JSON.stringify({
              error: "[DEBUG] Invalid token: missing offer_id or opp_id",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        if (oppId) {
          const { data: opp, error: oppError } = await supabaseAdmin
            .from("opportunities")
            .select(
              "id, student_name, student_phone, interests, slots_proposed",
            )
            .eq("id", oppId)
            .single();
          ...
          if (oppError || !opp) {
            return new Response(
              JSON.stringify({
                ...
                message: "Vaga não encontrada ou expirada.",
              }),
              {
                status: 410,
                headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if ((legacy_data || legacy_offer) && LEGACY_OFFER_DECODE) {
      ...
      try {
        ...
        const { error: legacyUsageError } = await supabaseAdmin.from(
          "legacy_offer_usage",
        ).insert({
          route: route || "unknown",
          raw_param: paramName,
        ...
        if (legacyUsageError) {
          console.warn(
            "legacy_offer_usage insert failed:",
            legacyUsageError.message,
          );
        }
        ...
        return new Response(
          JSON.stringify({
            ...
            payload,
            _warning:
              "This link uses a legacy format. It will stop working after the migration window.",
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      } catch (decodeError) {
        ...
        return new Response(
          JSON.stringify({
            error: "LEGACY_DECODE_FAILED",
            message: "Link inválido ou corrompido.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({
        error: "MISSING_TOKEN",
        message: "Nenhum token ou dado de oferta fornecido.",
      }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
...
function getErrorMessage(code: string | undefined): string {
  switch (code) {
    case "OFFER_NOT_FOUND":
      return "Este link não é válido. Solicite um novo.";
    case "OFFER_REVOKED":
      return "Este link foi revogado pela escola.";
    case "OFFER_EXPIRED":
      return "Este link expirou. Solicite um novo à escola.";
    default:
