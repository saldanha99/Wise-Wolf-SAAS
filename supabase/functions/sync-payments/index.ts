...
serve(async (req) => {
  ...
  try {
    ...
    if (!tenantId) {
      return new Response(
        JSON.stringify({
          error: "tenant_id_obrigatorio",
          detalhe: "Informe a escola para a qual gerar as mensalidades.",
        }),
        {
      ...
      if (!existing || existing.length === 0) {
        ...
        const { error: insError } = await supabase
          ...
          .insert({
            ...
            billing_type: "MANUAL",
            asaas_payment_id: `MANUAL_${Date.now()}_${
        ...
        if (insError) {
          logs.push(`Failed ${student.full_name}: ${insError.message}`);
        } else {
          createdCount++;
    ...
    return new Response(
      JSON.stringify({ success: true, created: createdCount, logs }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      },
