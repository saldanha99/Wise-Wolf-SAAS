...
serve(async (req) => {
  ...
  try {
    const auth = await authorizeRequest(req, {
      allowedRoles: ["SCHOOL_ADMIN", "SUPER_ADMIN", "COORDINATOR"],
      corsHeaders,
    });
    ...
    if (!caller?.tenant_id && caller?.role !== "SUPER_ADMIN") {
      return json({ error: "tenant required", slots: [] }, 403);
    }
    ...
    const date = /^\d{4}-\d{2}-\d{2}$/.test(String(payload.date ?? ""))
      ? String(payload.date)
      : null;
    const excludeTeacherId = payload.excludeTeacherId
      ? String(payload.excludeTeacherId)
      : null;

    if (!Number.isInteger(day) || day < 0 || day > 6 || !time) {
      return json({
        error: "parâmetros inválidos: 'day' (0-6) e 'time' (HH:MM)",
        slots: [],
      }, 400);
    }
    ...
    const { data: availabilities, error: availError } = await S
      ...
      .select(
        "teacher_id, start_time, end_time, profiles(full_name, phone, role, lifecycle_status)",
      )
      .eq("day_of_week", day)
    ...
    if (availError) {
      return json({
        error: "Availability DB Error: " + availError.message,
        slots: [],
      }, 500);
    }
    ...
    const candidates = (availabilities || []).filter((a: any) => {
      ...
      if (
        !p ||
        ["offboarded", "suspended"].includes(String(p.lifecycle_status || ""))
      ) return false;
    ...
    if (fixedErr) {
      return json(
        { error: "Booking DB Error: " + fixedErr.message, slots: [] },
        500,
      );
    }
    for (const b of (fixed || [])) {
      if (hhmm(b.time_slot) === time) busy.add(b.teacher_id);
    }
    ...
    if (date) {
      ...
      for (const b of (avulsas.data || [])) {
        if (hhmm(b.time_slot) === time) busy.add(b.teacher_id);
      }
      for (const r of (repos.data || [])) {
        if (hhmm(r.time) === time) busy.add(r.teacher_id);
      }
    }
    ...
    const slots = candidates
      ...
      .map((a: any) => {
        ...
        return {
          ...
          teacher_name: name,
          teacher_avatar: `https://ui-avatars.com/api/?name=${
