async function test() {
  ...
  const res = await fetch('https://dvalxbtngopxopzcbfdm.supabase.co/rest/v1/saas_leads?select=*&order=created_at.desc&limit=2', { headers: { 'apikey': anonKey, 'Authorization': 'Bearer ' + anonKey } });
  const data = await res.json();
  console.log('Recent SaaS Leads:', JSON.stringify(data, null, 2));
