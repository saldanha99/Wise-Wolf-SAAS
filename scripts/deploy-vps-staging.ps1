...
$stage = $null
$archive = $null
$apiUrl = (& ssh wisewolf-vps "grep '^VITE_SUPABASE_URL=' /opt/wisewolf/frontend/src/.env.production | head -n 1 | cut -d= -f2-").Trim()
...
$apiUri = [Uri]$apiUrl
if ($apiUri.Scheme -ne 'https' -or
    $apiUri.Host -ne 'api.wisewolflanguage.com.br' -or
    $apiUri.AbsolutePath -ne '/') {
  throw 'VITE_SUPABASE_URL deve apontar para a raiz da API pública da VPS.'
}
