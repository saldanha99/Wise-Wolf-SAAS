import { createHash } from 'node:crypto';
import { lstatSync, readFileSync, readdirSync } from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

export const HUB_PUBLIC_STORY_VIDEO_SLUGS = Object.freeze([
  'hub-overview',
  'library',
  'educator-ai',
  'wolfie',
  'school-os',
]);

const HUB_PUBLIC_STORY_VIDEO_COMPOSITIONS = Object.freeze({
  'hub-overview': 'HubOverviewPtBrStory',
  library: 'HubLibraryPtBrStory',
  'educator-ai': 'HubEducadorIaPtBrStory',
  wolfie: 'HubWolfiePtBrStory',
  'school-os': 'HubSchoolOsPtBrStory',
});

const SHA256_PATTERN = /^[a-f0-9]{64}$/u;
const STORY_ROOT = 'assets/hub/videos/social';
const STORY_MANIFEST_PATH = `${STORY_ROOT}/manifest.json`;
const STORY_MANIFEST_KEYS = Object.freeze([
  'video',
  'poster',
  'captions',
  'compositionId',
  'durationSeconds',
  'language',
  'receipt',
  'renderFingerprintSha256',
]);

const isPlainObject = (value) =>
  typeof value === 'object' && value !== null && !Array.isArray(value);

const hasExactKeys = (value, keys) =>
  isPlainObject(value)
  && Object.keys(value).sort().join('\0') === [...keys].sort().join('\0');

const stableSerialize = (value) => {
  if (Array.isArray(value)) return `[${value.map(stableSerialize).join(',')}]`;
  if (isPlainObject(value)) {
    return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${stableSerialize(value[key])}`).join(',')}}`;
  }
  const serialized = JSON.stringify(value);
  if (serialized === undefined) throw new Error('valor não serializável');
  return serialized;
};

const sha256Value = (value) =>
  createHash('sha256').update(stableSerialize(value)).digest('hex');

const sha256File = (filePath) =>
  createHash('sha256').update(readFileSync(filePath)).digest('hex');

export const isHubPublicStoryVideosEnabled = (value = process.env.VITE_HUB_PUBLIC_STORY_VIDEOS) =>
  value?.trim() === 'true';

export const getHubPublicStoryVideoArtifactPaths = (slug) => ({
  video: `${STORY_ROOT}/${slug}.mp4`,
  poster: `${STORY_ROOT}/posters/${slug}.webp`,
  captions: `${STORY_ROOT}/captions/${slug}.pt-BR.vtt`,
  receipt: `${STORY_ROOT}/receipts/${slug}.json`,
});

const resolveInside = (rootDirectory, relativePath) => {
  const resolvedRoot = path.resolve(rootDirectory);
  const resolvedPath = path.resolve(resolvedRoot, relativePath);
  if (resolvedPath !== resolvedRoot && !resolvedPath.startsWith(`${resolvedRoot}${path.sep}`)) {
    throw new Error(`artefato fora da raiz permitida: ${relativePath}`);
  }
  return resolvedPath;
};

const findDisabledPublicArtifacts = (rootDirectory) => {
  const storyRoot = resolveInside(rootDirectory, STORY_ROOT);
  let rootStats;
  try {
    rootStats = lstatSync(storyRoot);
  } catch {
    return [];
  }

  if (rootStats.isSymbolicLink() || !rootStats.isDirectory()) {
    return [`${STORY_ROOT}: raiz pública inválida`];
  }

  const blocked = [];
  const walk = (directory) => {
    const entries = readdirSync(directory, { withFileTypes: true })
      .sort((left, right) => left.name < right.name ? -1 : left.name > right.name ? 1 : 0);

    for (const entry of entries) {
      const absolutePath = path.join(directory, entry.name);
      const relativePath = path.relative(rootDirectory, absolutePath).split(path.sep).join('/');
      const extension = path.posix.extname(relativePath.toLowerCase());
      if (entry.isSymbolicLink()) {
        blocked.push(`${relativePath}: link simbólico não permitido`);
      } else if (entry.isDirectory()) {
        walk(absolutePath);
      } else if (entry.isFile() && (
        extension === '.mp4'
        || extension === '.webp'
        || (extension === '.json' && (relativePath.includes('/receipts/') || path.posix.basename(relativePath) === 'manifest.json'))
      )) {
        blocked.push(relativePath);
      }
    }
  };

  walk(storyRoot);
  return blocked;
};

const inspectArtifact = (rootDirectory, slug, kind, relativePath) => {
  const artifactPath = resolveInside(rootDirectory, relativePath);
  let stats;
  try {
    stats = lstatSync(artifactPath);
  } catch {
    return `${relativePath}: ausente`;
  }

  if (stats.isSymbolicLink() || !stats.isFile()) return `${relativePath}: não é um arquivo regular`;
  if (stats.size === 0) return `${relativePath}: arquivo vazio`;

  if (kind === 'captions') {
    const captions = readFileSync(artifactPath, 'utf8');
    if (!captions.startsWith('WEBVTT')) return `${relativePath}: legenda WebVTT inválida`;
  }

  if (kind === 'receipt') {
    try {
      const receipt = JSON.parse(readFileSync(artifactPath, 'utf8'));
      if (!hasExactKeys(receipt, [
        'schemaVersion',
        'slug',
        'generatedAt',
        'language',
        'compositionId',
        'commercialUseAllowed',
        'renderFingerprintSha256',
        'receiptFingerprintSha256',
        'artifacts',
      ])) return `${relativePath}: campos públicos do receipt inválidos`;
      if (!hasExactKeys(receipt.artifacts, ['videoSha256', 'posterSha256', 'captionsSha256'])) {
        return `${relativePath}: lista de artefatos inválida`;
      }
      if (receipt.schemaVersion !== 1 || receipt.slug !== slug) {
        return `${relativePath}: receipt não corresponde a ${slug}`;
      }
      if (receipt.language !== 'pt-BR'
        || receipt.compositionId !== HUB_PUBLIC_STORY_VIDEO_COMPOSITIONS[slug]) {
        return `${relativePath}: composição Story ou idioma inválido`;
      }
      if (receipt.commercialUseAllowed !== true) return `${relativePath}: licença comercial ausente`;
      if (!receipt.generatedAt || Number.isNaN(Date.parse(receipt.generatedAt))) {
        return `${relativePath}: data de geração inválida`;
      }
      if (!SHA256_PATTERN.test(receipt.renderFingerprintSha256)
        || !SHA256_PATTERN.test(receipt.receiptFingerprintSha256)) {
        return `${relativePath}: fingerprint inválido`;
      }
      const { receiptFingerprintSha256, ...receiptData } = receipt;
      if (sha256Value(receiptData) !== receiptFingerprintSha256) {
        return `${relativePath}: integridade do receipt inválida`;
      }

      const expectedArtifacts = getHubPublicStoryVideoArtifactPaths(slug);
      for (const [artifactKind, receiptKey] of [
        ['video', 'videoSha256'],
        ['poster', 'posterSha256'],
        ['captions', 'captionsSha256'],
      ]) {
        const recordedSha256 = receipt.artifacts[receiptKey];
        const expectedPath = resolveInside(rootDirectory, expectedArtifacts[artifactKind]);
        if (!SHA256_PATTERN.test(recordedSha256) || sha256File(expectedPath) !== recordedSha256) {
          return `${relativePath}: ${artifactKind} diverge do receipt`;
        }
      }
    } catch {
      return `${relativePath}: receipt JSON inválido`;
    }
  }

  return null;
};

const inspectManifest = (rootDirectory) => {
  const manifestPath = resolveInside(rootDirectory, STORY_MANIFEST_PATH);
  let stats;
  try {
    stats = lstatSync(manifestPath);
  } catch {
    return `${STORY_MANIFEST_PATH}: ausente`;
  }
  if (stats.isSymbolicLink() || !stats.isFile()) return `${STORY_MANIFEST_PATH}: não é um arquivo regular`;
  if (stats.size === 0) return `${STORY_MANIFEST_PATH}: arquivo vazio`;

  try {
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
    if (!hasExactKeys(manifest, ['generatedAt', 'format', 'width', 'height', 'aspectRatio', 'videos'])
      || !manifest.generatedAt
      || Number.isNaN(Date.parse(manifest.generatedAt))
      || manifest.format !== 'story'
      || manifest.width !== 1080
      || manifest.height !== 1920
      || manifest.aspectRatio !== '9:16') {
      return `${STORY_MANIFEST_PATH}: cabeçalho inválido`;
    }
    if (!hasExactKeys(manifest.videos, HUB_PUBLIC_STORY_VIDEO_SLUGS)) {
      return `${STORY_MANIFEST_PATH}: coleção de Stories inválida`;
    }

    for (const slug of HUB_PUBLIC_STORY_VIDEO_SLUGS) {
      const entry = manifest.videos[slug];
      if (!hasExactKeys(entry, STORY_MANIFEST_KEYS)) {
        return `${STORY_MANIFEST_PATH}: campos inválidos para ${slug}`;
      }
      const artifacts = getHubPublicStoryVideoArtifactPaths(slug);
      if (entry.video !== `/${artifacts.video}`
        || entry.poster !== `/${artifacts.poster}`
        || entry.captions !== `/${artifacts.captions}`
        || entry.receipt !== `/${artifacts.receipt}`
        || entry.compositionId !== HUB_PUBLIC_STORY_VIDEO_COMPOSITIONS[slug]
        || entry.language !== 'pt-BR'
        || !Number.isFinite(entry.durationSeconds)
        || entry.durationSeconds <= 0
        || !SHA256_PATTERN.test(entry.renderFingerprintSha256)) {
        return `${STORY_MANIFEST_PATH}: metadados inválidos para ${slug}`;
      }

      const receipt = JSON.parse(readFileSync(resolveInside(rootDirectory, artifacts.receipt), 'utf8'));
      if (entry.renderFingerprintSha256 !== receipt.renderFingerprintSha256) {
        return `${STORY_MANIFEST_PATH}: fingerprint diverge do receipt de ${slug}`;
      }
    }
  } catch {
    return `${STORY_MANIFEST_PATH}: JSON ou referência inválida`;
  }

  return null;
};

export const verifyHubPublicStoryVideoAssets = ({
  rootDirectory,
  enabled = isHubPublicStoryVideosEnabled(),
}) => {
  if (!rootDirectory) throw new Error('rootDirectory é obrigatório');
  if (!enabled) {
    const blockedArtifacts = findDisabledPublicArtifacts(rootDirectory);
    if (blockedArtifacts.length > 0) {
      throw new Error(
        `[Publicação dos Stories do Hub bloqueada]\n${blockedArtifacts.map((artifact) => `- ${artifact}: artefato público presente com VITE_HUB_PUBLIC_STORY_VIDEOS=false`).join('\n')}`,
      );
    }
    return { enabled: false, checked: 0, rootDirectory: path.resolve(rootDirectory) };
  }

  const failures = [];
  let checked = 0;
  for (const slug of HUB_PUBLIC_STORY_VIDEO_SLUGS) {
    const artifacts = getHubPublicStoryVideoArtifactPaths(slug);
    for (const [kind, relativePath] of Object.entries(artifacts)) {
      checked += 1;
      const failure = inspectArtifact(rootDirectory, slug, kind, relativePath);
      if (failure) failures.push(failure);
    }
  }
  checked += 1;
  const manifestFailure = inspectManifest(rootDirectory);
  if (manifestFailure) failures.push(manifestFailure);

  if (failures.length > 0) {
    throw new Error(
      `[Publicação dos Stories do Hub bloqueada]\n${failures.map((failure) => `- ${failure}`).join('\n')}`,
    );
  }

  return { enabled: true, checked, rootDirectory: path.resolve(rootDirectory) };
};

const isDirectExecution = process.argv[1]
  && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url);

if (isDirectExecution) {
  const args = process.argv.slice(2);
  const rootIndex = args.indexOf('--root');
  const enabledIndex = args.indexOf('--enabled');
  const rootDirectory = rootIndex >= 0 ? args[rootIndex + 1] : 'public';
  const enabled = enabledIndex >= 0
    ? isHubPublicStoryVideosEnabled(args[enabledIndex + 1])
    : isHubPublicStoryVideosEnabled();

  try {
    const result = verifyHubPublicStoryVideoAssets({ rootDirectory, enabled });
    if (result.enabled) {
      console.log(`Stories públicos do Hub validados: ${result.checked} artefatos em ${result.rootDirectory}`);
    } else {
      console.log('Stories públicos do Hub desativados e nenhum artefato publicável foi encontrado.');
    }
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
  }
}
