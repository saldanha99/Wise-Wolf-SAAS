...
Criar/configurar o aplicativo OAuth da própria escola no Google Cloud (feito em 26/09/2026, ver topo), habilitar Meet REST API e Google Drive API, registrar o redirect exato. Escopos: `openid`, `email`, `meetings.space.created`, `drive.meet.readonly` (restrito; é o que lê transcrição, notas e o relatório de presença — só arquivos criados pelo Meet). Não pedir acesso geral ao Gmail, calendário ou Drive.
