...
const PORT = 3003;
const AGENT_NAME = "🌐 TRANSLATOR AGENT";

const systemPrompt = `You are a highly skilled English-to-Brazilian-Portuguese translator.
YOUR ONLY JOB: Translate the given English text into natural Brazilian Portuguese.
RULES:
- Return ONLY the translated text. No explanations, no labels, no quotes.
- Keep the translation natural and colloquial (Brazilian Portuguese style).
- Preserve the conversational tone of the original.
- If the text already contains Portuguese, keep those parts as-is and translate only the English parts.`;

async function callGemini(apiKey, text) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`;
    const payload = {
        system_instruction: { parts: [{ text: systemPrompt }] },
        contents: [{ parts: [{ text: `Translate to Brazilian Portuguese:\n"${text}"` }] }],
    };
    ...
    if (req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                res.end(JSON.stringify({ agent: 'translator', result }));
            } catch (err) {
